DBSRW(RID,NOMSG,PGM)	;
	...
	;
	N dbtbl5h S dbtbl5h=$$vDb2("SYSDEV",RID)
	...
	.	;
	.	N libno,vop1 S libno=$$vDb5("SYSDEV","L",.vop1)
	.	I ($G(vop1)=0) S LIBNO=101
	.	E  S LIBNO=100+$P(libno,$C(124),1)
	.	;
	.	N dbtbl0 S dbtbl0=$$vDb3("SYSDEV","R")
	.	;
	.	N dbtbl5h S dbtbl5h=$$vDb2("SYSDEV",RID)
	.	 S vobj(dbtbl5h,0)=$G(^DBTBL(vobj(dbtbl5h,-3),5,vobj(dbtbl5h,-4),0))
	.	;
	.	I $P(vobj(dbtbl0),$C(124),1)="" S:'$D(vobj(dbtbl0,-100,"0*","DESC")) vobj(dbtbl0,-100,"0*","DESC")="N001"_$P(vobj(dbtbl0),$C(124),1) S vobj(dbtbl0,-100,"0*")="",$P(vobj(dbtbl0),$C(124),1)=1
	.	S NEXTRPT=$P(vobj(dbtbl0),$C(124),1) S:'$D(vobj(dbtbl0,-100,"0*","DESC")) vobj(dbtbl0,-100,"0*","DESC")="N001"_$P(vobj(dbtbl0),$C(124),1) S vobj(dbtbl0,-100,"0*")="",$P(vobj(dbtbl0),$C(124),1)=NEXTRPT+1
	.	S PGM="R"_$E(LIBNO,2,3)_"S"
	.	I NEXTRPT<1000 S PGM=PGM_$E(((1000+NEXTRPT)),2,4)
	.	E  S PGM=PGM_NEXTRPT
	. S:'$D(vobj(dbtbl5h,-100,0,"PGM")) vobj(dbtbl5h,-100,0,"PGM")="T002"_$P(vobj(dbtbl5h,0),$C(124),2) S vobj(dbtbl5h,-100,0)="",$P(vobj(dbtbl5h,0),$C(124),2)=PGM
	...
	. N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^DBTBL(vobj(dbtbl0,-3),0,vobj(dbtbl0,-4))=$$RTBAR^%ZFUNC(vobj(dbtbl0)) K vobj(dbtbl0,-100) S vobj(dbtbl0,-2)=1 Tcommit:vTp  
	.	;
	. Tcommit:$Tlevel 
	.	K vobj(+$G(dbtbl0)),vobj(+$G(dbtbl5h)) Q 
	;
	; Re-load record after potential updates above
	K vobj(+$G(dbtbl5h)) S dbtbl5h=$$vDb2("SYSDEV",RID)
	 S vobj(dbtbl5h,0)=$G(^DBTBL(vobj(dbtbl5h,-3),5,vobj(dbtbl5h,-4),0))
	;
...
BLDV0(dbtbl5h,RPTINFO,ddmap,VARLIST)	;
	...
	N TABCHR S TABCHR=$char(9)
	;
	; Access report through job queue at V0 entry point
	D addcode^DBSRWUTL(0,"V0"_TABCHR_"// External report entry point") ; job queue
	D addcode^DBSRWUTL(0,"")
	;
	...
	D addcode^DBSRWUTL(3,"#ACCEPT Date=07/26/06; Pgm=RussellDS; CR=22121; Group=MISMATCH")
	...
	D addcode^DBSRWUTL(1,"if 'vcrt set IOSL="_$P(vobj(dbtbl5h,0),$C(124),6)_TABCHR_"// Non-interactive")
	D addcode^DBSRWUTL(1,"else  do {"_TABCHR_"// Interactive")
	D addcode^DBSRWUTL(2,"do TERM^%ZUSE(IO,""WIDTH="_(RPTINFO("RSIZE")+1)_""")")
	D addcode^DBSRWUTL(2,"write $$CLEARXY^%TRMVT")
	I RPTINFO("RSIZE")>80 D addcode^DBSRWUTL(2,"write $$SCR132^%TRMVT"_TABCHR_"// Switch to 132 col mode")
	D addcode^DBSRWUTL(2,"}")
	;
	D addcode^DBSRWUTL(0,"")
	;
...
BLDBANNR(PQINFO,RPTINFO,dbtbl5h)	;
	...
	D addcode^DBSRWUTL(0,"")
	;
	N docrs,vos1,vos2,vos3,vos4  N V1 S V1=vobj(dbtbl5h,-4) S docrs=$$vOpen1()
	S SEQ=1
	F  Q:'($$vFetch1())  D
...
CMPALL	; Private - Mass compile client/server reports
	...
	D vDbDe1()
	;
	N rptrs,vos1,vos2 S rptrs=$$vOpen3()
	;
	F  Q:'($$vFetch3())  D
	.	N RID
	.	S RID=rptrs
	.	N tmpdq,vop1,vop2,vop3 S tmpdq="",vop2=$J,vop1=RID,vop3=0
	.	;
...
GETRWRS(INPUT,VARS,ddmap)	;
	...
	;
vSIG()	;
	Q "60680^48228^Dan Russell^39031" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vStrFnd(object,p1,p2,p3,qt)	; String.find
	...
	Q p2
	;
vDb2(v1,v2)	;	vobj()=Db.getRecord(DBTBL5H,,0)
	;
	N vOid
	...
	Q vOid
	;
vDb3(v1,v2)	;	vobj()=Db.getRecord(DBTBL0,,1)
	;
	N vOid
	S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordDBTBL0"
	S vobj(vOid)=$G(^DBTBL(v1,0,v2))
	I vobj(vOid)="",'$D(^DBTBL(v1,0,v2))
	S vobj(vOid,-2)='$T
	;
	S vobj(vOid,-3)=v1
	S vobj(vOid,-4)=v2
	Q vOid
	;
vDb4()	;	voXN = Db.getRecord(CUVAR,,0)
	;
	I '$D(^CUVAR)
	I $T S $ZS="-1,"_$ZPOS_",%PSL-E-RECNOFL,,CUVAR" X $ZT
	Q ""
	;
vDb5(v1,v2,v2out)	;	voXN = Db.getRecord(DBTBL0,,1,-2)
	;
	N libno
	S libno=$G(^DBTBL(v1,0,v2))
	I libno="",'$D(^DBTBL(v1,0,v2))
	S v2out='$T
	;
	Q libno
	;
...
vL1a1	S vos2=$G(V1) I vos2="" G vL1a0
	S vos3=30.999
vL1a3	S vos3=$O(^DBTBL("SYSDEV",5,vos2,vos3),1) I vos3=""!(111']]vos3) G vL1a0
	I '(vos3]]90.999) G vL1a3
	Q
	;
vFetch1()	;
	;
	...
	I vos1=0 Q 0
	;
	S vos4=$G(^DBTBL("SYSDEV",5,vos2,vos3))
	S docrs=$P(vos4,$C(12),1)
	;
	Q 1
	;
...
vL3a0	S vos1=0 Q
vL3a1	S vos2=""
vL3a2	S vos2=$O(^DBTBL("SYSDEV",5,vos2),1) I vos2="" G vL3a0
	I '(vos2'?1"Z".E) G vL3a2
	I '(vos2'?1"z".E) G vL3a2
	Q
	;
vFetch3()	;
...
vOpen5()	;	DATA FROM DBTBL13D WHERE LIBS='SYSDEV' AND PID=:PPID ORDER BY SEQ ASC
	...
	;
vL5a0	S vos1=0 Q
vL5a1	S vos2=$G(PPID) I vos2="" G vL5a0
	S vos3=0
vL5a3	S vos3=$O(^DBTBL("SYSDEV",13,vos2,vos3),1) I vos3="" G vL5a0
	Q
	;
vFetch5()	;
	;
	...
	I vos1=0 Q 0
	;
	S vos4=$G(^DBTBL("SYSDEV",13,vos2,vos3))
	S rs=$P(vos4,$C(12),1)
	;
	Q 1
	;
...
vtrap1	;	Error trap
	...
	E  USE 0 WRITE !!,RID,?15,$P(error,",",2),",",$P(error,",",3),",",$P(error,",",4),!
	Q 
