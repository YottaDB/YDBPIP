...
patchparse(code,expr,var,newPtr)	; parse and patch code
	...
	...			I code[x S code=$piece(code,x,1)_$E(($piece(code,x,2)),2,$L(code))
	...			Q 
	..		Q 
	.	;
	...
	.	S code=$piece(code,z)_label_"(."_p0_","_p1_")"_$piece(code,z,2)
	.	Q 
	;
	S y=oLvn_"("
...
patchNew(var,new,newPtr)	; private void; Patch list new command line
	...
	I '(z["N ") D  Q 
	.	S f="S "_var
	.	S z=$piece(z,f,1)_"N "_$E(new,2,1048575)_f_$piece(z,f,2,99)
	.	S msrc(newPtr)=z
	.	Q 
	I '(z[f) D  ; N x,var syntax instead
	.	S f=","_var
	.	I '(z[f) S new=" N "_$E(new,2,1048575)
	.	Q 
	S z=$piece(z,f,1)_f_new_$piece(z,f,2,999)
	S msrc(newPtr)=z
	Q 
	;
	; ---------------------------------------------------------------------
...
ResultSet(subRou,var,newPtr,flag)	;
	...
	;
	S n="" S new=""
	F  S n=$order(patch(-3,subRou,var,newPtr,n)) Q:n=""  S new=new_","_patch(-3,subRou,var,newPtr,n)
	;
	I 'flag,sort>1 D
	.	S new=new_",vOid"
...
change(code,test,subRou,var,newPtr)	; Change resultSet generate code
	...
	;
vSIG()	;
	Q "60576^22069^Frans S.C. Witte^31604" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
