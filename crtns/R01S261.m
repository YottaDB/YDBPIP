R01S261	; DBTBL14 - Data Item Protection Listing
	;
	; **** Routine compiled from DATA-QWIK Report DBTBL14 ****
	;
	...
	;
	S RID="DBTBL14"
	S RN="Data Item Protection Listing"
	I $get(IO)="" S IO=$I
	;
	D INIT^%ZM()
	;
	S %TAB("IO")=$$IO^SCATAB
	S %TAB("VIN2")="|255||[DBTBL14Q]FID|[DBTBL14Q]FID:DISTINCT:NOVAL||D EXT^DBSQRY||T|File Name|||||"
	S %TAB("VIN3")="|255||[DBTBL14Q]DINAM|[DBTBL14Q]DINAM:DISTINCT:NOVAL||D EXT^DBSQRY||T|Data Item|||||"
	;
	S %READ="IO/REQ,VIN2#0,VIN3#0,"
	;
	; Skip device prompt option
	I $get(VRWOPT("NOOPEN")) S %READ="VIN2#0,VIN3#0,"
	;
	S VFMQ=""
...
V0	; External report entry point
	...
	N VWHERE
	N %TIM N CONAM N RID N RN N VL N VLOF N VRF N VSTATS N vCOL N vHDG N vc1 N vc10 N vc11 N vc12 N vc2 N vc3 N vc4 N vc5 N vc6 N vc7 N vc8 N vc9 N vovc1 N vovc2 N vovc3 N vovc4 N vovc5 N vrundate N vsysdate
	;
	N cuvar S cuvar=$$vDb2()
	 S cuvar=$G(^CUVAR("BANNER"))
	;
	S CONAM="DATA-QWIK 7.2"
	S ER=0 S RID="DBTBL14" S RN="Data Item Protection Listing"
	S VL=""
	;
	...
	..		N CHRSET S CHRSET=$$^UCIOENCD("Report","DBTBL14","V0","*")
	...
	D INIT^%ZM()
	;
	S vCOL="[DBTBL14Q]QRYDESC#27#100"
	;
	; Build WHERE clause to use for dynamic query
	D
	.	N SEQ S SEQ=1
	.	N DQQRY N FROM
	.	S DQQRY(1)="[DBTBL14Q]PLIBS = ""SYSDEV""" S SEQ=SEQ+1
	.	I $get(VIN2)="" S VIN2="ALL"
	.	I VIN2'="ALL" S DQQRY(2)="[DBTBL14Q]FID "_VIN2 S SEQ=SEQ+1
	.	I $get(VIN3)="" S VIN3="ALL"
	.	I VIN3'="ALL" S DQQRY(SEQ)="[DBTBL14Q]DINAM "_VIN3 S SEQ=SEQ+1
	.	S FROM=$$DQJOIN^SQLCONV("DBTBL14Q,DBTBL14") Q:ER 
	...
	.	N VBNRINFO
	.	;
	.	S VBNRINFO("PROMPTS",2)="WC2|"_"File Name"_"|VIN2|"_$get(VIN2)
	.	S VBNRINFO("PROMPTS",3)="WC2|"_"Data Item"_"|VIN3|"_$get(VIN3)
	.	;
	.	D
	..		N SEQ
	..		N VALUE N VAR N X
	...
	..		Q 
	.	;
	.	S VBNRINFO("DESC")="Data Item Protection Listing"
	.	S VBNRINFO("PGM")="R01S261"
	.	S VBNRINFO("RID")="DBTBL14"
	.	S VBNRINFO("TABLES")="DBTBL14Q,DBTBL14"
	.	;
	.	S VBNRINFO("ORDERBY",1)="[SYSDEV,DBTBL14Q]PLIBS"
	.	S VBNRINFO("ORDERBY",2)="[SYSDEV,DBTBL14Q]14"
	.	S VBNRINFO("ORDERBY",3)="[SYSDEV,DBTBL14Q]FID"
	.	S VBNRINFO("ORDERBY",4)="[SYSDEV,DBTBL14Q]DINAM"
	.	S VBNRINFO("ORDERBY",5)="[SYSDEV,DBTBL14Q]GROUP"
	.	S VBNRINFO("ORDERBY",6)="[SYSDEV,DBTBL14Q]QUERY"
	.	;
	.	D ^DBSRWBNR(IO,.VBNRINFO) ; Print banner
	.	Q 
	;
	; Initialize variables
	S (vc1,vc2,vc3,vc4,vc5,vc6,vc7,vc8,vc9,vc10,vc11,vc12)=""
	...
	N rwrs,vos1,vos2,sqlcur,exe,sqlcur,vd,vi,vsql,vsub S rwrs=$$vOpen0(.exe,.vsql,"DBTBL14Q.PLIBS,DBTBL14Q.FID,DBTBL14Q.DINAM,DBTBL14Q.GROUP,DBTBL14Q.QUERY,DBTBL14.FILDESC,DBTBL14.PROTPGM,DBTBL14.ITEMDESC,DBTBL14.DESC,DBTBL14.PROTOPT,DBTBL14.RUCLS1,DBTBL14Q.QRYDESC","DBTBL14Q,DBTBL14",VWHERE,"DBTBL14Q.PLIBS,DBTBL14Q.FID,DBTBL14Q.DINAM,DBTBL14Q.GROUP,DBTBL14Q.QUERY","","/DQMODE=1")
...
VSAVLAST	; Save last access keys values
	...
	;
VGETDATA(V,VI)	;
	S vc1=$piece(V,$char(9),1) ; DBTBL14Q.PLIBS
	S vc2=$piece(V,$char(9),2) ; DBTBL14Q.FID
	S vc3=$piece(V,$char(9),3) ; DBTBL14Q.DINAM
	S vc4=$piece(V,$char(9),4) ; DBTBL14Q.GROUP
	S vc5=$piece(V,$char(9),5) ; DBTBL14Q.QUERY
	S vc6=$piece(V,$char(9),6) ; DBTBL14.FILDESC
	S vc7=$piece(V,$char(9),7) ; DBTBL14.PROTPGM
	S vc8=$piece(V,$char(9),8) ; DBTBL14.ITEMDESC
	S vc9=$piece(V,$char(9),9) ; DBTBL14.DESC
	S vc10=$piece(V,$char(9),10) ; DBTBL14.PROTOPT
	S vc11=$piece(V,$char(9),11) ; DBTBL14.RUCLS1
	S vc12=$piece(V,$char(9),12) ; DBTBL14Q.QRYDESC
	Q 
	;
...
VHDG4	; Group Header
	...
	;
	S VL="  "_$E(vc2,1,30)
	S VL=VL_$J("",35-$L(VL))_$E(vc6,1,40)
	S VL=VL_$J("",94-$L(VL))_$E(vc7,1,8)
	D VOM
	Q 
	;
VDTL4	; Detail
	;
	Q:VD(4)  S VD(4)=1 ; Print flag
	I VLC+2>IOSL D VHDG0 Q:VFMQ  S vh(4)=0 D VHDG4 Q:VFMQ 
	;
	S VL="        "_"-------------------------------------------------------------------------------------------------------------------------"
	D VOM
	S VL="  "_$E(vc3,1,30)
	S VL=VL_$J("",35-$L(VL))_$E(vc8,1,40)
	D VOM
	Q 
	;
VDTL5	; Detail
	;
	Q:VD(5)  S VD(5)=1 ; Print flag
	I VLC+3>IOSL D VHDG0 Q:VFMQ 
	;
	S VL="" D VOM
	S VL="             "_"#"
	S VL=VL_$J("",15-14)_$J(vc4,2)
	S VL=VL_$J("",19-$L(VL))_"Desc: "
	S VL=VL_$J("",26-$L(VL))_$E(vc9,1,50)
	S VL=VL_$J("",81-$L(VL))_$E(vc10,1,10)
	D VOM
	S VL="                   "_"UCLS: "
	S VL=VL_$J("",26-25)_$E(vc11,1,50)
	D VOM
	Q 
	;
VDTL6	; Detail
	...
	;
	S VL="                  "_"Query:"
	S VL=VL_$J("",26-24)_$E(vc12,1,100)
	D VOM
	Q 
	;
VHDG0	; Page Header
	...
	D VOM
	S VL="" D VOM
	S VL="  "_"File Name                        Description                                               Protection Name"
	D VOM
	S VL="===================================================================================================================================="
	D VOM
	;
