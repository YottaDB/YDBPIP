DBSRW3(dbtbl5h,ddmap,RPTINFO)	;
	...
	...			I V'?1A.AN S V="("_V_")"
	...			E  I NEWLIST'[(","_V_",") S NEWLIST=NEWLIST_V_","
	...			S v=V
...
BUILDSEC(LVL,SEC,PGTRRGN,RPTINFO,ddmap,SKIPLF)	;
	...
	.	;
	.	S NEWLIST=$$newlist^DBSRWUTL("ALL",.RPTINFO,.ddmap,NEWLIST)
	.	F  D  Q:(NEWLIST="") 
	...
	..		I NEWLIST'="" D addcode^DBSRWUTL(1,"type Public String "_NEWLIST)
	...
	....				;
	....				I 'PPFLG D  Q 
	.....					I SKIPLF S CODE="set VL=VL_$J("""","_(L-1)_"-VL.length())_"_Z
	.....					E  I L=1 S CODE="set VL="_Z ; On column one
	.....					E  S CODE="set VL="""_$J("",L-1)_"""_"_Z
	.....					Q 
	....				E  D  ; With pre/post proc
	.....					I SKIPLF S CODE=Z_"set VL=VL_$J("""","_(L-1)_"-VL.length())_V"
	...
	...			E  D
	....				I 'PPFLG D
	.....					S CODE="set VL=VL_$J("""","_(L-1)_"-VL.length())_"_Z
	...
	...			E  D
	....				S CODE="set VL=VL_$J("_""""""_","_(L-1)_"-VL.length())_V"
	....				I FLDOVF S CODE=CODE_",VOFFLG=0"
	....				Q 
...
STAT1(REF,LVL,RPTINFO,ddmap,VXCNT)	;
	...
	..		D addcode^DBSRWUTL(1,CODE)
	..		S CODE="set "_VX_"=$S("_VX2_":"_VX_"/"_VX2_",1:0)"
	...
	I FUN="AVG" Q "$S(VT("_LVL_"):VT("_LVL_","_SEQ_",1)/VT("_LVL_"),1:0)" ;@AVG
	...
	.	N dbtbl1d,vos1,vos2,vos3,vos4 S dbtbl1d=$$vOpen1()
...
BRKINFO(dbtbl5h,ddmap,RPTINFO,BRKCODE)	;
	...
	.	N dbtbl1d S dbtbl1d=$$vDb4("SYSDEV",TABLE,COLUMN)
...
BLDPRNT(RPTINFO)	; Private - Build print section
	...
	.	D addcode^DBSRWUTL(1,"write VL_$J("""","_RPTINFO("RSIZE")_"-VL.length()),!")
	...
	.	D addcode^DBSRWUTL(2,"if 'AUXPTR.get() write $C(12),!")
	...
	.	D addcode^DBSRWUTL(2,"set VL=VL_$J("""",LOC+LN-VL.length()-V.length())_V")
	.	D addcode^DBSRWUTL(2," set VOFFLG=0")
	.	D addcode^DBSRWUTL(2,"}")
	.	D addcode^DBSRWUTL(1,"else  do {")
	.	D addcode^DBSRWUTL(2,"set VLOF=VLOF_$J("""",LOC+LN-V.length()-VLOF.length())_V")
	...
	..		D addcode^DBSRWUTL(3,"if VR>1 set VRF(VSEQ)=VRF(VSEQ)_$J("""",((VR-1)*"_RPTINFO("REPEATSIZE")_")-VRF(VSEQ).length())")
	...
	..		D addcode^DBSRWUTL(1,"for  do { quit:DONE")
	..		D addcode^DBSRWUTL(2,"if VR<"_(RPTINFO("REPEATCNT")+1)_" do { quit")
	..		D addcode^DBSRWUTL(3,"if VR>1 set VRF=VRF_$J("""",((VR-1)*"_RPTINFO("REPEATSIZE")_")-VRF.length())")
	...
	;
vSIG()	;
	Q "60885^65355^e0101874^29974" ; Signature - LTD^TIME^USER^SIZE
	;
...
vL1a3	S vos3=$O(^DBINDX("SYSDEV","STR",vos2,vos3),1) I vos3="" G vL1a0
	S vos4=""
vL1a5	S vos4=$O(^DBINDX("SYSDEV","STR",vos2,vos3,1,vos4),1) I vos4="" G vL1a3
	Q
	;
vFetch1()	;
	;
