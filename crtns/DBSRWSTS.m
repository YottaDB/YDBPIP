DBSRWSTS(VSTATS)	;
	...
	.	;
	.	N src S src=$$vDb2("SYSDEV",TABLE,COLUMN)
	.	;
	.	D VOM
	.	;
	...
	..		S TABLE=$piece(TC,".",1) S COLUMN=$piece(TC,".",2)
	..		N trc S trc=$$vDb2("SYSDEV",TABLE,COLUMN)
...
ACCUM(CNT,AMT,TYP,BTYP)	;
	...
	S TOTAMT=TOTAMT+AMT
	;
	S VL=VL_$J($$CONV("N",$J(CNT*100/TOPCNT,0,2)),8)
	;
	; TOTAL & % TOTALS
	I BTYP'="" S TYP=BTYP
	Q:'TOPAMT 
	...
	S VL=VL_$J($$CONV(TYP,$J(AMT*100/TOPAMT,0,2)),8)
	;
	Q 
	;
...
CONV(TYP,INPUT)	;
	...
	E  I TYP="E"!(TYP="$") S NEWVAL=$J($FN(INPUT,",",2),2)
...
VBRSAVE(LINE,DATA)	; Private - Save for report browser
	...
	;
vSIG()	;
	Q "60107^24115^sviji^6238" ; Signature - LTD^TIME^USER^SIZE
	;
vDb2(v1,v2,v3)	;	voXN = Db.getRecord(DBTBL1D,,0)
	;
	N src
	S src=$G(^DBTBL(v1,1,v2,9,v3))
	I src="",'$D(^DBTBL(v1,1,v2,9,v3))
