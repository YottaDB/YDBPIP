DBS2PSL4(dbtbl2)	;DBS2PSL4; - V7.0 - PSL Screen compiler
	...
	K OM
	N rs1,vos4,vos5,vos6 S rs1=$$vOpen2()
	F  Q:'($$vFetch2())  D
	.	N code N pseq
	.	N dbtbl2pp,vop2 S vop2=$P(rs1,$C(9),4),dbtbl2pp=$G(^DBTBL($P(rs1,$C(9),1),2,$P(rs1,$C(9),2),$P(rs1,$C(9),3),vop2))
	.	S pseq=vop2
	.	S code=$P(dbtbl2pp,$C(12),1)
	.	S OM(pseq)=code
	.	Q  ; while ...
	...
	S TMPZ(1)=PGM_"(Number %ProcessMode,"_vobjlst("formal")_")   //"_$P(vobj(dbtbl2,0),$C(124),11)_" - "_$P(vobj(dbtbl2,0),$C(124),12)_" - SID= <"_SID_"> "_$P(vobj(dbtbl2,0),$C(124),9)
	D ^SCACOPYR(.X1)
	S TMPZ(2)=X1
	K X1
	;
	...
	F  Q:'($$vFetch3())  D
	.	N code,pseq
	.	N dbtbl2pp,vop3 S vop3=$P(rs2,$C(9),4),dbtbl2pp=$G(^DBTBL($P(rs2,$C(9),1),2,$P(rs2,$C(9),2),$P(rs2,$C(9),3),vop3))
	.	S pseq=vop3
	.	S code=$P(dbtbl2pp,$C(12),1)
	.	S TMPZ(X1)=" // "_code
	.	S X1=X1+.01
	...
	I vspp D
	.	S VZSEQ=40 S X=""
	.	N pproc,vos10,vos11,vos12 S pproc=$$vOpen4()
	.	F  Q:'($$vFetch4())  D
	..		N code,pseq
	..		N dbtbl2pp,vop4 S vop4=$P(pproc,$C(9),4),dbtbl2pp=$G(^DBTBL($P(pproc,$C(9),1),2,$P(pproc,$C(9),2),$P(pproc,$C(9),3),vop4))
	..		S pseq=vop4
	..		S code=$P(dbtbl2pp,$C(12),1)
	..		S diset(pseq)=code
	..		Q  ; while ...
...
PPUTIL(node,tag)	;
	...
	F  Q:'($$vFetch5())  D
	.	N code N pseq
	.	N dbtbl2pp,vop1 S vop1=$P(rs,$C(9),4),dbtbl2pp=$G(^DBTBL($P(rs,$C(9),1),2,$P(rs,$C(9),2),$P(rs,$C(9),3),vop1))
	.	S pseq=vop1
	.	S code=$P(dbtbl2pp,$C(12),1)
	.	S OM(pseq)=code
	.	Q  ; while ...
...
PPLIB1(ppnam,code,i)	;
	...
	N ds,vos1,vos2,vos3  N V2 S V2=xpp S ds=$$vOpen6()
	F  Q:'($$vFetch6())  D
	.	N dbtbl13d S dbtbl13d=$G(^DBTBL($P(ds,$C(9),1),13,$P(ds,$C(9),2),$P(ds,$C(9),3)))
...
vREPEAT(di,field)	;
	;
	I ($D(REPEAT($$vStrUC(di)))#2) Q 1 ; referenced column is in repeat region
	;
	N fid
...
setkeys(dbtbl2,KEYS)	;
	...
	S count=0
	;
	N file,vop1,vop2,vop3,vop4 S vop1="SYSDEV",vop2=pfid,file=$$vDb4("SYSDEV",pfid)
...
Primary(pfid)	; File list from screen control page /REQ/MECH=VAL
	...
	F i=1:1:$L(pfid,",") D
	.	S fid=$piece(pfid,",",i)
	.	N f,vop1,vop2,vop3 S vop1="SYSDEV",vop2=fid,f=$$vDb4("SYSDEV",fid)
...
DEFAULT(fid,code)	;
	...
	K code
	N dbtbl1,vop1,vop2,vop3,vop4,vop5 S vop1="SYSDEV",vop2=fid,dbtbl1=$$vDb4("SYSDEV",fid)
	 S vop5=$G(^DBTBL(vop1,1,vop2,101))
	 S vop3=$G(^DBTBL(vop1,1,vop2,12))
	 S vop4=$G(^DBTBL(vop1,1,vop2,16))
	S vpc=(($P(vop5,$C(124),1)="")) Q:vpc  ; No defaults
	S keys=""
...
vSIG()	;
	Q "60779^35705^chenardp^33970" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vDb4(v1,v2)	;	voXN = Db.getRecord(DBTBL1,,0)
	;
	N file
	S file=$G(^DBTBL(v1,1,v2))
	I file="",'$D(^DBTBL(v1,1,v2))
...
vDbEx10()	;	min(1): DISTINCT LIBS,SID,SEQ,PSEQ FROM DBTBL2PP WHERE LIBS=:%LIBS AND SID=:SID AND SEQ=0 AND PSEQ=41
	...
	S vsql1=$G(%LIBS) I vsql1="" Q 0
	;
	I '($D(^DBTBL(vsql1,2,SID,0,41))#2) Q 0
	Q 1
	;
...
vDbEx2()	;	min(1): DISTINCT LIBS,SID,SEQ,PSEQ FROM DBTBL2PP WHERE LIBS=:%LIBS AND SID=:SID AND SEQ=0 AND PSEQ=21
	...
	S vsql1=$G(%LIBS) I vsql1="" Q 0
	;
	I '($D(^DBTBL(vsql1,2,SID,0,21))#2) Q 0
	Q 1
	;
...
vDbEx7()	;	min(1): DISTINCT LIBS,SID,SEQ,PSEQ FROM DBTBL2PP WHERE LIBS=:%LIBS AND SID=:SID AND SEQ=0 AND PSEQ=1
	...
	S vsql1=$G(%LIBS) I vsql1="" Q 0
	;
	I '($D(^DBTBL(vsql1,2,SID,0,1))#2) Q 0
	Q 1
	;
...
vL1a1	S vos2=$G(SID) I vos2="" G vL1a0
	S vos3=69.999
	I $D(^DBTBL("SYSDEV",2,vos2,0,vos3)),'(vos3>80.999) G vL1a5
vL1a4	S vos3=$O(^DBTBL("SYSDEV",2,vos2,0,vos3),1) I vos3=""!(vos3>80.999) G vL1a0
vL1a5	Q
	;
vFetch1()	;
	;
...
vL2a1	S vos5=$G(SID) I vos5="" G vL2a0
	S vos6=120.999
	I $D(^DBTBL("SYSDEV",2,vos5,0,vos6)),'(vos6>140.999) G vL2a5
vL2a4	S vos6=$O(^DBTBL("SYSDEV",2,vos5,0,vos6),1) I vos6=""!(vos6>140.999) G vL2a0
vL2a5	Q
	;
vFetch2()	;
	;
	...
	I vos4=0 Q 0
	;
	S rs1="SYSDEV"_$C(9)_vos5_$C(9)_0_$C(9)_$S(vos6=$$BYTECHAR^SQLUTL(254):"",1:vos6)
	;
	Q 1
	;
vOpen3()	;	LIBS,SID,SEQ,PSEQ FROM DBTBL2PP WHERE LIBS='SYSDEV' AND SID=:SID AND SEQ=0 AND PSEQ BETWEEN 80.999 AND 100.999
	...
	;
vL3a0	S vos7=0 Q
vL3a1	S vos8=$G(SID) I vos8="" G vL3a0
	S vos9=80.999
	I $D(^DBTBL("SYSDEV",2,vos8,0,vos9)),'(vos9>100.999) G vL3a5
vL3a4	S vos9=$O(^DBTBL("SYSDEV",2,vos8,0,vos9),1) I vos9=""!(vos9>100.999) G vL3a0
vL3a5	Q
	;
vFetch3()	;
	;
	...
	I vos7=0 Q 0
	;
	S rs2="SYSDEV"_$C(9)_vos8_$C(9)_0_$C(9)_$S(vos9=$$BYTECHAR^SQLUTL(254):"",1:vos9)
	;
	Q 1
	;
vOpen4()	;	LIBS,SID,SEQ,PSEQ FROM DBTBL2PP WHERE LIBS='SYSDEV' AND SID=:SID AND SEQ=0 AND PSEQ BETWEEN 41 AND 60
	...
	;
vL4a0	S vos10=0 Q
vL4a1	S vos11=$G(SID) I vos11="" G vL4a0
	S vos12=41
	I $D(^DBTBL("SYSDEV",2,vos11,0,vos12)),'(vos12>60) G vL4a5
vL4a4	S vos12=$O(^DBTBL("SYSDEV",2,vos11,0,vos12),1) I vos12=""!(vos12>60) G vL4a0
vL4a5	Q
	;
vFetch4()	;
	;
	...
	I vos10=0 Q 0
	;
	S pproc="SYSDEV"_$C(9)_vos11_$C(9)_0_$C(9)_$S(vos12=$$BYTECHAR^SQLUTL(254):"",1:vos12)
	;
	Q 1
	;
...
vL5a1	S vos2=$G(SID) I vos2="" G vL5a0
	S vos3=$G(X) I vos3="",'$D(X) G vL5a0
	S vos4=$G(X2) I vos4="",'$D(X2) G vL5a0
	S vos5=vos3
	I $D(^DBTBL("SYSDEV",2,vos2,0,vos5)),'(vos5>vos4) G vL5a7
vL5a6	S vos5=$O(^DBTBL("SYSDEV",2,vos2,0,vos5),1) I vos5=""!(vos5>vos4) G vL5a0
vL5a7	Q
	;
vFetch5()	;
	;
	...
	I vos1=0 Q 0
	;
	S rs="SYSDEV"_$C(9)_vos2_$C(9)_0_$C(9)_$S(vos5=$$BYTECHAR^SQLUTL(254):"",1:vos5)
	;
	Q 1
	;
...
vL6a1	S vos2=$G(V2) I vos2="" G vL6a0
	S vos3=0
vL6a3	S vos3=$O(^DBTBL("SYSDEV",13,vos2,vos3),1) I vos3="" G vL6a0
	Q
	;
vFetch6()	;
	;
