...
BUILD(FID)	; Table name
	...
	F  Q:'($$vFetch1())  D  Q:ER 
	.	;
	.	N dbtbl14,vop5,vop6 S vop6=$P(ds,$C(9),3),vop5=$P(ds,$C(9),4),dbtbl14=$G(^DBTBL($P(ds,$C(9),1),14,$P(ds,$C(9),2),vop6,vop5))
	.	;
	.	S DI=vop6
	.	;
	...
	Q 
	;
BUILDDI(FID,DINAM,OBJNAME,SORT,KEYLIST,QSEQ,SUBSLIST)	;
	;
	N ER S ER=0
	...
	.	Q 
	E  D
	.	N dbtbl1d S dbtbl1d=$$vDb5("SYSDEV",FID,DINAM)
	.	;
	...
	..		I UCLSWHR'["NOT" S INPUT("WHERE")="NOT ("_UCLSWHR_")"
	..		E  S INPUT("WHERE")=UCLSWHR
	..		;
	...
	Q ER
	;
BLDQRYS(FID,DINAM,GROUP,SORT,SUBSLIST)	;
	;
	N ER S ER=0
	N SEQ
	...
	Q 0
	;
BLDSUBS(OBJNAME,SUBSLIST)	;
	;
	N VAR S VAR=""
	;
	F  S VAR=$order(SUBSLIST("VAR",VAR)) Q:(VAR="")  D
	.	D addcode(1,"set "_VAR_" = "_OBJNAME_"."_$$vStrLC(SUBSLIST("VAR",VAR),0))
	.	Q 
	;
	D addcode(0,"")
	;
...
INSTCODE(FID,OBJNAME,KEYLIST)	;
	...
	Q CODE
	;
NEWLIST(DI,GROUP,SORT,SUBSLIST)	;
	;
...
UCLS2WHR(RUCLS,WHERE,RM)	;
	...
	..		Q 
	.	;
	.	I NOT S WHERE=WHERE_"(:%UCLS NOT "_OPER_" "_RIGHT_")"
	.	E  S WHERE=WHERE_"(:%UCLS "_OPER_" "_RIGHT_")"
	.	Q 
	;
...
VER(fid)	; Table name
	...
	;
	N rs,vos1,vos2,vos3,vos4  N V1 S V1=fid S rs=$$vOpen3()
	;
...
addcode(TABS,CODE)	;
	...
	;
vSIG()	;
	Q "60788^54134^Badrinath Giridharan^29925" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vL1a1	S vos2=$G(FID) I vos2="" G vL1a0
	S vos3=""
vL1a3	S vos3=$O(^DBTBL("SYSDEV",14,vos2,vos3),1) I vos3="" G vL1a0
	S vos4=""
vL1a5	S vos4=$O(^DBTBL("SYSDEV",14,vos2,vos3,vos4),1) I vos4="" G vL1a3
	Q
	;
vFetch1()	;
	;
...
vL2a1	S vos2=$G(FID) I vos2="" G vL2a0
	S vos3=$G(DINAM) I vos3="" G vL2a0
	S vos4=$G(GROUP) I vos4="" G vL2a0
	S vos5=""
vL2a5	S vos5=$O(^DBTBL("SYSDEV",14,vos2,vos3,vos4,vos5),1) I vos5="" G vL2a0
	S vos6=$G(^DBTBL("SYSDEV",14,vos2,vos3,vos4,vos5))
	I '($P(vos6,"|",1)'="") G vL2a5
	Q
	;
vFetch2()	;
