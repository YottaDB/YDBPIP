...
SETHDR	; Private - Copy current RW page header information into vHDG() array
	...
	.	Q 
	;
	N rs2,vos5,vos6,vos7,vos8,vos9  N V2 S V2=$J S rs2=$$vOpen2()
	;
	S K=1
	F  Q:'($$vFetch2())  D
	...
	S vHDG=K_"|"_PGNO
	;
	N tmprptbr,vop1,vop2,vop3,vop4,vop5 S vop4=$J,vop3=0,vop2=PGNO,vop1=0,tmprptbr=$$vDb2($J,0,PGNO,0,.vop5)
...
BOTTOM	; Private - Last Page
	...
	..		N rs,vos1,vos2,vos3  N V1 S V1=$J S rs=$$vOpen3()
	..		;
	..		I '$G(vos1) S PAGE=1
...
NEWHDR(PAGE)	; Private - Switch to new header
	...
	K vHDG
	;
	N rs,vos1,vos2,vos3,vos4,vos5  N V1 S V1=$J S rs=$$vOpen4()
	F  Q:'($$vFetch4())  D
	.	S vHDG(I)=rs
	.	S I=I+1
	.	Q 
...
PNTALL(LN1,LN2)	;
	...
	N rs,vos1,vos2,vos3  N V1 S V1=$J S rs=$$vOpen5()
	F  Q:'($$vFetch5())  D
	.	N PAGE S PAGE=rs
	.	;
...
FIND1	; Private
	...
	.	N rs,vos1,vos2,vos3,vos4,vos5,vos6  N V1 S V1=$J S rs=$$vOpen6()
	.	F  Q:'($$vFetch6())  D  Q:HIT 
	..		N X
	..		S X=rs
	..		;    #ACCEPT DATE=01/02/03;PGM=Dan Russell;CR=Mark Spier
...
FINDHDR(LN1,LN2)	;
	...
	..		N rs,vos1,vos2,vos3,vos4  N V1 S V1=$J S rs=$$vOpen7()
	..		I '$G(vos1) S HDRPTR=1
...
TMPGET(LINE,PAGE,SEQ,DATA)	;
	...
	; Change to private once that feature works in PSL
	;
	N tmprptbr,vop1 S tmprptbr=$$vDb2($J,LINE,PAGE,SEQ,.vop1)
	;
	S DATA=$P(tmprptbr,$C(12),1)
...
TMPSET(LINE,PAGE,SEQ,DATA)	;
	;
	N tmprptbr,vop1,vop2,vop3,vop4,vop5 S vop4=$J,vop3=LINE,vop2=PAGE,vop1=SEQ,tmprptbr=$$vDb2($J,LINE,PAGE,SEQ,.vop5)
	;
	S $P(tmprptbr,$C(12),1)=DATA
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^TMPRPTBR(vop4,vop3,vop2,vop1)=tmprptbr S vop5=1 Tcommit:vTp  
	Q 
...
LASTPAGE()	; Private - Return highest page number
	...
	N PAGE S PAGE=0
	;
	N rs,vos1,vos2,vos3,vos4  N V1 S V1=$J S rs=$$vOpen8()
	;
	I $$vFetch8() S PAGE=rs
	;
	Q PAGE
...
LASTLINE()	; Return highest line number
	...
	N LINE S LINE=1
	;
	N rs,vos1,vos2,vos3,vos4,vos5  N V1 S V1=$J S rs=$$vOpen9()
	;
	I $$vFetch9() S LINE=rs
	;
	Q LINE
	;
vSIG()	;
	Q "60842^35326^Dan Russell^18165" ; Signature - LTD^TIME^USER^SIZE
	;
vDb2(v1,v2,v3,v4,v2out)	;	voXN = Db.getRecord(TMPRPTBR,,1,-2)
	;
	N tmprptbr
	S tmprptbr=$G(^TMPRPTBR(v1,v2,v3,v4))
	I tmprptbr="",'$D(^TMPRPTBR(v1,v2,v3,v4))
	S v2out='$T
	;
	Q tmprptbr
	;
...
vL1a1	S vos2=$G(V1) I vos2="" G vL1a0
	S vos3=""
vL1a3	S vos3=$O(^TMPRPTBR(vos2,0,vos3),-1) I vos3="" G vL1a0
	I '($D(^TMPRPTBR(vos2,0,vos3,0))#2) G vL1a3
	Q
	;
vFetch1()	;
	;
	...
	I vos1=0 Q 0
	;
	S vos4=$G(^TMPRPTBR(vos2,0,vos3,0))
	S rs=$S(vos3=$C(254):"",1:vos3)_$C(9)_$P(vos4,$C(12),1)
	;
	Q 1
	;
vOpen2()	;	LINENO,DATA FROM TMPRPTBR WHERE JOBNO=:V2 AND LINENO >= :J AND PAGENO=0 AND SEQ=0 ORDER BY LINENO ASC
	...
	;
vL2a0	S vos5=0 Q
vL2a1	S vos6=$G(V2) I vos6="" G vL2a0
	S vos7=$G(J) I vos7="",'$D(J) G vL2a0
	S vos8=vos7
	I $D(^TMPRPTBR(vos6,vos8)) G vL2a6
vL2a5	S vos8=$O(^TMPRPTBR(vos6,vos8),1) I vos8="" G vL2a0
vL2a6	I '($D(^TMPRPTBR(vos6,vos8,0,0))#2) G vL2a5
	Q
	;
vFetch2()	;
	;
	...
	I vos5=0 Q 0
	;
	S vos9=$G(^TMPRPTBR(vos6,vos8,0,0))
	S rs2=$S(vos8=$C(254):"",1:vos8)_$C(9)_$P(vos9,$C(12),1)
	;
	Q 1
	;
...
vL3a1	S vos2=$G(V1) I vos2="" G vL3a0
	S vos3=""
vL3a3	S vos3=$O(^TMPRPTBR(vos2,0,vos3),-1) I vos3="" G vL3a0
	I '($D(^TMPRPTBR(vos2,0,vos3,1))#2) G vL3a3
	Q
	;
vFetch3()	;
	;
...
vL4a1	S vos2=$G(V1) I vos2="" G vL4a0
	S vos3=$G(PAGE) I vos3="" G vL4a0
	S vos4=0
vL4a4	S vos4=$O(^TMPRPTBR(vos2,0,vos3,vos4),1) I vos4="" G vL4a0
	Q
	;
vFetch4()	;
	;
	...
	I vos1=0 Q 0
	;
	S vos5=$G(^TMPRPTBR(vos2,0,vos3,vos4))
	S rs=$P(vos5,$C(12),1)
	;
	Q 1
	;
...
vL5a1	S vos2=$G(V1) I vos2="" G vL5a0
	S vos3=""
vL5a3	S vos3=$O(^TMPRPTBR(vos2,0,vos3),1) I vos3="" G vL5a0
	I '($D(^TMPRPTBR(vos2,0,vos3,1))#2) G vL5a3
	Q
	;
vFetch5()	;
	;
...
vL6a1	S vos2=$G(V1) I vos2="" G vL6a0
	S vos3=$G(LN1) I vos3="",'$D(LN1) G vL6a0
	S vos4=$G(LN2) I vos4="",'$D(LN2) G vL6a0
	S vos5=vos3
	I $D(^TMPRPTBR(vos2,vos5)),'(vos5>vos4) G vL6a7
vL6a6	S vos5=$O(^TMPRPTBR(vos2,vos5),1) I vos5=""!(vos5>vos4) G vL6a0
vL6a7	I '($D(^TMPRPTBR(vos2,vos5,0,0))#2) G vL6a6
	Q
	;
vFetch6()	;
	;
	...
	I vos1=0 Q 0
	;
	S vos6=$G(^TMPRPTBR(vos2,vos5,0,0))
	S rs=$P(vos6,$C(12),1)
	;
	Q 1
	;
...
vL7a1	S vos2=$G(V1) I vos2="" G vL7a0
	S vos3=$G(PGNO) I vos3="",'$D(PGNO) G vL7a0
	S vos4=vos3
	I $D(^TMPRPTBR(vos2,0,vos4)) G vL7a6
vL7a5	S vos4=$O(^TMPRPTBR(vos2,0,vos4),-1) I vos4="" G vL7a0
vL7a6	I '($D(^TMPRPTBR(vos2,0,vos4,1))#2) G vL7a5
	Q
	;
vFetch7()	;
	;
...
vL8a1	S vos2=$G(V1) I vos2="" G vL8a0
	S vos3=""
vL8a3	S vos3=$O(^TMPRPTBR(vos2,0,vos3),-1) I vos3="" G vL8a0
	S vos4=""
vL8a5	S vos4=$O(^TMPRPTBR(vos2,0,vos3,vos4),1) I vos4="" G vL8a3
	Q
	;
vFetch8()	;
	;
...
vL9a1	S vos2=$G(V1) I vos2="" G vL9a0
	S vos3=""
vL9a3	S vos3=$O(^TMPRPTBR(vos2,vos3),-1) I vos3="" G vL9a0
	S vos4=""
vL9a5	S vos4=$O(^TMPRPTBR(vos2,vos3,vos4),1) I vos4="" G vL9a3
	S vos5=""
vL9a7	S vos5=$O(^TMPRPTBR(vos2,vos3,vos4,vos5),1) I vos5="" G vL9a5
	Q
	;
vFetch9()	;
	;
