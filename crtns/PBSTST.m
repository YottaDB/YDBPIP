...
ztrap	;Private;Setup PBSSRV to trap all messages to ZTRAP global
	...
	;
	I ($D(^TRAP1("UID"))#2) D
	.	S vop1="UID",trap1u=$$vDb12("UID",.vop2)
	.	S UID=$P(trap1u,$C(124),1)
	.	Q 
	;
	I ($D(^TRAP1("PGM"))#2) D
	.	S vop3="PGM",trap1p=$$vDb12("PGM",.vop4)
	.	S SVPGM=$P(trap1p,$C(124),1)
	.	Q 
	;
	N trap1n,vop5,vop6 S vop5="NMS",trap1n=$$vDb13("NMS",.vop6)
	S NMS=$P(trap1n,$C(124),1)
	;
	N trap1d,vop7,vop8 S vop7="DTS",trap1d=$$vDb13("DTS",.vop8)
	...
	I ((SVTYP=""))!((SVID="")) Q 
	;
	S vop1="UID",trap1u=$$vDb13("UID",.vop2)
	S $P(trap1u,$C(124),1)=UID
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^TRAP1(vop1)=$$RTBAR^%ZFUNC(trap1u) S vop2=1 Tcommit:vTp  
	;
	S vop3="PGM",trap1p=$$vDb13("PGM",.vop4)
	S $P(trap1p,$C(124),1)=SVPGM
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^TRAP1(vop3)=$$RTBAR^%ZFUNC(trap1p) S vop4=1 Tcommit:vTp  
	;
	S $P(trap1n,$C(124),1)=NMS
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^TRAP1(vop5)=$$RTBAR^%ZFUNC(trap1n) S vop6=1 Tcommit:vTp  
	;
	S $P(trap1d,$C(124),1)=$P($H,",",1)_","_$P($H,",",2)
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^TRAP1(vop7)=$$RTBAR^%ZFUNC(trap1d) S vop8=1 Tcommit:vTp  
	;
	N rs
	I SVTYP="*" S rs=$$vOpen2()
	...
	.	;
	.	N svctrl,vop9,vop10,vop11,vop12 S svctrl="",vop11="",vop10="",vop9="",vop12=0
	. S vop11=SVTYP
	. S vop10=SVID
	. S vop9=$O(^SVCTRL(SVTYP,SVID,""),-1)+1
	. S $P(svctrl,$C(124),1)="TRAP=0"
	. N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^SVCTRL(vop11,vop10,vop9)=$$RTBAR^%ZFUNC(svctrl) S vop12=1 Tcommit:vTp  
	.	;
	.	I OPER="ENABLE" D
...
ztrapm	;
	...
	..		S quit=1
	..		N svtrap S svtrap=$$vDb6($J)
	..	 S $P(vobj(svtrap),$C(124),1)=1
	..	 N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" D vReSav1(svtrap) S vobj(svtrap,-2)=1 Tcommit:vTp  
	..		K vobj(+$G(svtrap)) Q 
	.	;
	...
	.	Q 
	;
	N trap0 S trap0=$$vDb7($J,0)
	S seq=$P(vobj(trap0),$C(124),1)+1
	I seq>50 S seq=1
	;
	S $P(vobj(trap0),$C(124),1)=seq_" PBSSRV"
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" D vReSav2(trap0) S vobj(trap0,-2)=1 Tcommit:vTp  
	;
	 N V1,V2 S V1=$J,V2=seq K ^TRAP(V1,V2)
	;
	N trap0s S trap0s=$$vDbNew2("","")
	 S vobj(trap0s,1,1)=""
	S vobj(trap0s,-3)=$J
	S vobj(trap0s,-4)=seq
	S $P(vobj(trap0s),$C(124),1)="M:"_$$vdat2str(($P($H,",",1)_","_$P($H,",",2)),"MM/DD/YY 24:60:SS")_"|"_vzermsg
	S vobj(trap0s,1,1)=vzpkt
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" D vReSav2(trap0s) S vobj(trap0s,-2)=1 Tcommit:vTp  
	;
...
zchkuid(mtok)	;
	...
	I ($get(mtok)="") Q 1
	;
	N token,vop1 S token=$$vDb14(mtok,.vop1)
	I $G(vop1)=0 Q 1
	;
...
xtrap	; Index ^ZTRAP entries into XTRAP
	;
	N rs,vos1,vos2,vos3 S rs=$$vOpen5()
	;
	...
	.	S x=$P(rs,$C(9),1)
	.	S y=$P(rs,$C(9),2)
	.	N trap0,vop1,vop2,vop3 S vop1=x,vop2=y,trap0=$$vDb15(x,y)
...
REPLAY(SERVER,SEQ)	;
	...
	N token,vop1,vop2 S vop1=HDR(2),token=$$vDb14(vop1,.vop2)
	I $G(vop2) D
	. S $P(token,$C(124),1)=1
	...
	;
vSIG()	;
	Q "60529^22256^SChhabria^18402" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vDbDe1()	; DELETE FROM MSGLOG WHERE TOKEN=:V1 and MSGID=:V2
	...
	Q 
	;
vDb10()	;	voXN = Db.getRecord(DDPSTS,,0)
	;
	I '$D(^%ZDDP)
	I $T S $ZS="-1,"_$ZPOS_",%PSL-E-RECNOFL,,DDPSTS" X $ZT
	Q ""
	;
vDb11(v1)	;	voXN = Db.getRecord(SCAU,,0)
	;
	N scau
	S scau=$G(^SCAU(1,v1))
	I scau="",'$D(^SCAU(1,v1))
	...
	Q scau
	;
vDb12(v1,v2out)	;	voXN = Db.getRecord(TRAP1,,0,-2)
	;
	N trap1u
	S trap1u=$G(^TRAP1(v1))
	I trap1u="",'$D(^TRAP1(v1))
	S v2out=1
	I $T S $ZS="-1,"_$ZPOS_",%PSL-E-RECNOFL,,TRAP1" X $ZT
	Q trap1u
	;
vDb13(v1,v2out)	;	voXN = Db.getRecord(TRAP1,,1,-2)
	;
	N trap1u
	S trap1u=$G(^TRAP1(v1))
	I trap1u="",'$D(^TRAP1(v1))
	...
	Q trap1u
	;
vDb14(v1,v2out)	;	voXN = Db.getRecord(TOKEN,,1,-2)
	;
	N token
	S token=$G(^TOKEN(v1))
	I token="",'$D(^TOKEN(v1))
	...
	Q token
	;
vDb15(v1,v2)	;	voXN = Db.getRecord(TRAP0,,0)
	;
	N trap0
	S trap0=$G(^TRAP(v1,v2))
	I trap0="",'$D(^TRAP(v1,v2))
	I $T S $ZS="-1,"_$ZPOS_",%PSL-E-RECNOFL,,TRAP0" X $ZT
	Q trap0
	;
vDb6(v1)	;	vobj()=Db.getRecord(SVTRAP,,1)
	;
	N vOid
	...
	Q vOid
	;
vDb7(v1,v2)	;	vobj()=Db.getRecord(TRAP0,,1)
	;
	N vOid
	S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordTRAP0"
	S vobj(vOid)=$G(^TRAP(v1,v2))
	I vobj(vOid)="",'$D(^TRAP(v1,v2))
	S vobj(vOid,-2)='$T
	;
	S vobj(vOid,-3)=v1
	S vobj(vOid,-4)=v2
	Q vOid
	;
vDbNew2(v1,v2)	;	vobj()=Class.new(TRAP0)
	;
	N vOid
	 S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordTRAP0",vobj(vOid,-2)=0,vobj(vOid)=""
	S vobj(vOid,-3)=v1
	S vobj(vOid,-4)=v2
	Q vOid
	;
vFetch(vRs)	;	Runtime fetch
	...
	if vTag=("$$vFetch2^"_vPgm) quit $$vFetch2(vRs)
	if vTag=("$$vFetch3^"_vPgm) quit $$vFetch3(vRs)
...
vL1a0	S vos1=0 Q
vL1a1	S vos2=""
vL1a2	S vos2=$O(^SVCTRL("SCA$IBS",vos2),-1) I vos2="" G vL1a0
	Q
	;
vFetch1()	;
	;
...
vL2a1	S vobj(vOid,1)=""
vL2a2	S vobj(vOid,1)=$O(^SVCTRL(vobj(vOid,1)),1) I vobj(vOid,1)="" G vL2a0
	S vobj(vOid,2)=""
vL2a4	S vobj(vOid,2)=$O(^SVCTRL(vobj(vOid,1),vobj(vOid,2)),1) I vobj(vOid,2)="" G vL2a2
	Q
	;
vFetch2(vOid)	;
	;
	...
	I vobj(vOid,0)=0 Q 0
	;
	S vobj(vOid)=$S(vobj(vOid,1)=$C(254):"",1:vobj(vOid,1))_$C(9)_$S(vobj(vOid,2)=$C(254):"",1:vobj(vOid,2))
	;
	Q 1
	;
...
vL3a1	S vobj(vOid,1)=$G(SVTYP) I vobj(vOid,1)="" G vL3a0
	S vobj(vOid,2)=""
vL3a3	S vobj(vOid,2)=$O(^SVCTRL(vobj(vOid,1),vobj(vOid,2)),1) I vobj(vOid,2)="" G vL3a0
	Q
	;
vFetch3(vOid)	;
	;
	...
	I vobj(vOid,0)=0 Q 0
	;
	S vobj(vOid)=vobj(vOid,1)_$C(9)_$S(vobj(vOid,2)=$C(254):"",1:vobj(vOid,2))
	;
	Q 1
	;
vOpen4()	;	SVTYP,SVID FROM SVCTRLT WHERE SVTYP=:X ORDER BY SVID
	...
	;
vL4a0	S vos1=0 Q
vL4a1	S vos2=$G(X) I vos2="" G vL4a0
	S vos3=""
vL4a3	S vos3=$O(^SVCTRL(vos2,vos3),1) I vos3="" G vL4a0
	Q
	;
vFetch4()	;
	;
...
vL5a0	S vos1=0 Q
vL5a1	S vos2=""
vL5a2	S vos2=$O(^TRAP(vos2),1) I vos2="" G vL5a0
	S vos3=0
vL5a4	S vos3=$O(^TRAP(vos2,vos3),1) I vos3="" G vL5a2
	Q
	;
vFetch5()	;
	;
	...
	I vos1=0 Q 0
	;
	S rs=$S(vos2=$C(254):"",1:vos2)_$C(9)_$S(vos3=$C(254):"",1:vos3)
	;
	Q 1
	;
vOpen6()	;	MESSAGE FROM TRAP0 WHERE PID=:SERVER AND SEQ=:SEQ
	...
	;
vL6a0	S vos1=0 Q
vL6a1	S vos2=$G(SERVER) I vos2="" G vL6a0
	S vos3=$G(SEQ) I vos3="" G vL6a0
	I '($D(^TRAP(vos2,vos3))#2) G vL6a0
	Q
	;
vFetch6()	;
	;
	...
	;
	S vos1=100
	S vos4=$$READ^DBSMEMO("^TRAP("_vos2_","_vos3_")")
	S rs=vos4
	S vos1=0
	;
	Q 1
...
vL7a1	S vos2=$G(V1) I vos2="" G vL7a0
	S vos3=$G(V2) I vos3="" G vL7a0
	I '($D(^MSGLOG(vos2,vos3))#2) G vL7a0
	Q
	;
vFetch7()	;
	;
	...
	Q 1
	;
vReSav1(svtrap)	;	RecordSVTRAP saveNoFiler()
	;
	S ^SVTRAP(vobj(svtrap,-3))=$$RTBAR^%ZFUNC($G(vobj(svtrap)))
	N vC,vS s vS=0
	F vC=1:450:$L($G(vobj(svtrap,1,1))) S vS=vS+1,^SVTRAP(vobj(svtrap,-3),vS)=$E(vobj(svtrap,1,1),vC,vC+449)
	Q
	;
vReSav2(trap0)	;	RecordTRAP0 saveNoFiler()
	;
	S ^TRAP(vobj(trap0,-3),vobj(trap0,-4))=$$RTBAR^%ZFUNC($G(vobj(trap0)))
	N vC,vS s vS=0
	F vC=1:450:$L($G(vobj(trap0,1,1))) S vS=vS+1,^TRAP(vobj(trap0,-3),vobj(trap0,-4),vS)=$E(vobj(trap0,1,1),vC,vC+449)
	Q
	;
vtrap1	;	Error trap
	;
