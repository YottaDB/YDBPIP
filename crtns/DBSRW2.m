DBSRW2(dbtbl5h,ddmap,RPTINFO,PQINFO)	;
	...
	.	D addcode^DBSRWUTL(1,"type Public String "_$TR(X,"="""))
...
MEMO	; Private - Set up code to print memo - break across lines if necessary
	...
	D addcode^DBSRWUTL(2,"set VL=VL_$J("""",TAB-VL.length())_X do VOM")
	D addcode^DBSRWUTL(2,"set VL=$J("""",TAB)")
	D addcode^DBSRWUTL(1,"}")
	D addcode^DBSRWUTL(1,"set VL=$J("""",VLLEN)")
...
BLDGETD(ddmap,RPTINFO)	;
	...
	.	S CODE="set "_VARNAME_"=V.piece($C(9),"_I_")"_$char(9)_"// "_TC
	.	D addcode^DBSRWUTL(1,CODE)
...
ISVAR(X)	; Private - Return 1 if X is a valid variable name
	...
	;
vSIG()	;
	Q "60619^39698^Dan Russell^33277" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vL1a1	S vos2=$G(RID) I vos2="" G vL1a0
	S vos3="@"
vL1a3	S vos3=$O(^DBTBL("SYSDEV",5,vos2,vos3),1) I vos3="" G vL1a0
	S vos4=$G(^DBTBL("SYSDEV",5,vos2,vos3,25))
	I '($P(vos4,"|",3)>0) G vL1a3
	Q
	;
vFetch1()	;
	;
	...
	I vos1=0 Q 0
	;
	S rs=$S(vos3=$C(254):"",1:vos3)_$C(9)_$P(vos4,"|",1)_$C(9)_$P(vos4,"|",2)_$C(9)_$P(vos4,"|",3)_$C(9)_$P(vos4,"|",4)
	;
	Q 1
