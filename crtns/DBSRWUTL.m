...
addcode(TABS,LINE,LINENO)	;
	...
	.	I CMT'="" D
	..		I LEN<55 S LINE=LINE_$E(($C(9,9,9,9,9,9,9,9,9,9)),1,((63-LEN)\8)-1)
	...
	..		Q:($translate(CODE,$C(9,32))="")  ; Ignore blank lines
...
addvars(LIST,VAR,RPTINFO,REMOVE)	;
	...
	.	F  Q:'($$vFetch1())  D
	..		N syskeywd,vop1 S vop1=ds,syskeywd=$G(^STBL("SYSKEYWORDS",vop1))
...
coldefs(dbtbl5h,RPTINFO)	;
	...
	.	N dbtbl5dg,vop1,vop2,vop3 S vop1="SYSDEV",vop2=vobj(dbtbl5h,-4),vop3=KEY,dbtbl5dg=$$vDb7("SYSDEV",vop2,KEY)
	...
	.	S LN=0
	.	;
	.	N ds,vos1,vos2,vos3,vos4  N V2 S V2=vobj(dbtbl5h,-4) S ds=$$vOpen2()
...
direfs(dbtbl5h,RPTINFO,ddmap)	;
	...
	;
	N ds1,vos1,vos2,vos3,vos4  N V1 S V1=vobj(dbtbl5h,-4) S ds1=$$vOpen3()
	F  Q:'($$vFetch3())  D
	.	N TC
	.	N dbtbl5d S dbtbl5d=$G(^DBTBL($P(ds1,$C(9),1),5,$P(ds1,$C(9),2),$P(ds1,$C(9),3),$P(ds1,$C(9),4)))
	.	I $$validtcr($P(dbtbl5d,$C(124),6),.TC) D addtomap(TC,.ddmap)
	.	D COMPLEX($P(dbtbl5d,$C(124),7),.FILES,.ddmap)
	.	Q 
	;
	N ds2,vos5,vos6,vos7,vos8,vos9  N V2 S V2=vobj(dbtbl5h,-4) S ds2=$$vOpen4()
	F  Q:'($$vFetch4())  D
	.	N dbtbl5d1 S dbtbl5d1=$G(^DBTBL($P(ds2,$C(9),1),5,$P(ds2,$C(9),2),$P(ds2,$C(9),3),$P(ds2,$C(9),4),$P(ds2,$C(9),5)))
...
BLDFMT(LVL,KEY,RPTINFO,ddmap)	;
	...
	N REGINFO
	;
	N dbtbl5dg,vop1,vop2,vop3,vop4,vop5,vop6 S vop1="SYSDEV",vop2=RPTINFO("RID"),vop3=KEY,dbtbl5dg=$$vDb8("SYSDEV",vop2,KEY,.vop4)
	 S dbtbl5dg=$G(^DBTBL(vop1,5,vop2,vop3,0))
	...
	N ds1,vos1,vos2,vos3,vos4  N V1 S V1=RPTINFO("RID") S ds1=$$vOpen5()
	F  Q:'($$vFetch5())  D  Q:ER 
	.	N LN N LNCOL N PPSEQ N PRESEQ N SIZE
	.	N BLANK N FORMAT N ORIGTAG N PP N PPID N PRE N PREV N SEC N TAG N VAR
	.	N dbtbl5d,vop7,vop8,vop9 S vop9=$P(ds1,$C(9),2),vop8=$P(ds1,$C(9),3),vop7=$P(ds1,$C(9),4),dbtbl5d=$G(^DBTBL($P(ds1,$C(9),1),5,vop9,vop8,vop7))
	.	;
	.	S (PPSEQ,PRESEQ)=""
	.	;
...
GETCPPS(RID,GRP,ITMSEQ,SEQ,PPID,RPTINFO,ddmap)	;
	...
	S CNT=1
	;
	N rs,vos1,vos2,vos3,vos4,vos5,vos6,vos7,vos8 S rs=$$vOpen6()
	...
	.	S PPIN(CNT)=DATA
	.	S CNT=CNT+1
	.	I (($E($$vStrUC(DATA),$L(DATA)-11+1,$L(DATA))="TYPE PUBLIC")) D
	..		N I
	..		;
...
RPTSEQ(RID,GRP)	;
	...
	I GRP="@PH" Q 1
	;
	N dbtbl5h,vop1,vop2,vop3,vop4,vop5,vop6,vop7,vop8,vop9,vop10,vop11,vop12 S vop1="SYSDEV",vop2=RID,dbtbl5h=$$vDb9("SYSDEV",RID)
...
SORTFLG(dbtbl5h)	;
	...
	;
	N dbtbl1,vop1,vop2,vop3 S vop1="SYSDEV",vop2=PFID,dbtbl1=$$vDb10("SYSDEV",PFID)
...
vstats(SEQ,SRCVAL,TRGTVAL,STATINFO,VSTATS)	;
	...
	;
vSIG()	;
	Q "60842^35370^Dan Russell^36510" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vDb10(v1,v2)	;	voXN = Db.getRecord(DBTBL1,,0)
	...
	Q dbtbl1
	;
vDb7(v1,v2,v3)	;	voXN = Db.getRecord(DBTBL5DGC,,1)
	;
	I '(v3]]"@")
	...
	Q ""
	;
vDb8(v1,v2,v3,v2out)	;	voXN = Db.getRecord(DBTBL5DGC,,1,-2)
	;
	I '(v3]]"@")
	E  I '$D(^DBTBL(v1,5,v2,v3))
	S v2out='$T
	...
	Q ""
	;
vDb9(v1,v2)	;	voXN = Db.getRecord(DBTBL5H,,0)
	;
	N dbtbl5h
...
vL3a1	S vos2=$G(V1) I vos2="" G vL3a0
	S vos3=""
vL3a3	S vos3=$O(^DBTBL("SYSDEV",5,vos2,vos3),1) I vos3="" G vL3a0
	S vos4=100
vL3a5	S vos4=$O(^DBTBL("SYSDEV",5,vos2,vos3,vos4),1) I vos4="" G vL3a3
	Q
	;
vFetch3()	;
	;
	...
	I vos1=0 Q 0
	;
	S ds1="SYSDEV"_$C(9)_vos2_$C(9)_$S(vos3=$C(254):"",1:vos3)_$C(9)_$S(vos4=$C(254):"",1:vos4)
	;
	Q 1
	;
vOpen4()	;	LIBS,RID,GRP,ITMSEQ,SEQ FROM DBTBL5D1 WHERE LIBS='SYSDEV' AND RID=:V2
	...
	;
vL4a0	S vos5=0 Q
vL4a1	S vos6=$G(V2) I vos6="" G vL4a0
	S vos7=""
vL4a3	S vos7=$O(^DBTBL("SYSDEV",5,vos6,vos7),1) I vos7="" G vL4a0
	S vos8=100
vL4a5	S vos8=$O(^DBTBL("SYSDEV",5,vos6,vos7,vos8),1) I vos8="" G vL4a3
	S vos9=""
vL4a7	S vos9=$O(^DBTBL("SYSDEV",5,vos6,vos7,vos8,vos9),1) I vos9="" G vL4a5
	Q
	;
vFetch4()	;
	;
	...
	I vos5=0 Q 0
	;
	S ds2="SYSDEV"_$C(9)_vos6_$C(9)_$S(vos7=$C(254):"",1:vos7)_$C(9)_$S(vos8=$C(254):"",1:vos8)_$C(9)_$S(vos9=$C(254):"",1:vos9)
	;
	Q 1
	;
...
vL5a1	S vos2=$G(V1) I vos2="" G vL5a0
	S vos3=$G(KEY) I vos3="" G vL5a0
	S vos4=100
vL5a4	S vos4=$O(^DBTBL("SYSDEV",5,vos2,vos3,vos4),1) I vos4="" G vL5a0
	Q
	;
vFetch5()	;
	;
	...
	I vos1=0 Q 0
	;
	S ds1="SYSDEV"_$C(9)_vos2_$C(9)_vos3_$C(9)_$S(vos4=$C(254):"",1:vos4)
	;
	Q 1
	;
vOpen6()	;	DATA FROM DBTBL5D1 WHERE LIBS='SYSDEV' AND RID=:RID AND GRP=:GRP AND ITMSEQ=:ITMSEQ AND SEQ>:START AND SEQ<:END ORDER BY SEQ ASC
	...
	;
vL6a0	S vos1=0 Q
vL6a1	S vos2=$G(RID) I vos2="" G vL6a0
	S vos3=$G(GRP) I vos3="" G vL6a0
	S vos4=$G(ITMSEQ) I vos4="" G vL6a0
	S vos5=$G(START) I vos5="",'$D(START) G vL6a0
	S vos5=+vos5
	S vos6=$G(END) I vos6="",'$D(END) G vL6a0
	S vos6=+vos6
	I '(vos4>100) G vL6a0
	S vos7=vos5
vL6a10	S vos7=$O(^DBTBL("SYSDEV",5,vos2,vos3,vos4,vos7),1) I vos7=""!(vos7'<vos6) G vL6a0
	Q
	;
vFetch6()	;
	;
	...
	I vos1=0 Q 0
	;
	S vos8=$G(^DBTBL("SYSDEV",5,vos2,vos3,vos4,vos7))
	S rs=$P(vos8,$C(12),1)
	;
	Q 1
