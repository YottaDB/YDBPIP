...
INQ	; Inquiry mode
	;--------------------------------------------------------------------
	;
	S %OSAVE=$get(%O) S %O=$SELECT($get(%O)=4:4,1:2)
	D START Q 
	;
	;--------------------------------------------------------------------
DEL	; Delete mode
...
START	;
	...
	;
	; I18N=OFF
	I $D(%BLK) S %IPMODE="NOINT:ORDER %BLK/DELIM="_$ascii(%BLK,2)
...
BUILD(P,vnum)	; Build screen by parsing %TAB or [FID]DI syntax
	...
	..		Q 
	.	N rs,vos3,vos4,vos5 S rs=$$vOpen2()
	...
	I vlen="" S vlen=$SELECT("N$"[vtyp:12,"DC"[vtyp:10,vtyp="L":1,1:255)
	;
	I 'vsiz S vsiz=vlen
	I vdes'="" S vdes=vdes_":"
...
BLDVO(vrepeat)	; Build VO(array) and %TAB(array)
	...
	S vop=vop+1 S VO(vop)=$char(vcy)_$char(N)_$char(vl)_$char(vreq)_"00000001T"_vdes
	;
	I vl S vcx=vcx+1
	...
	;
	S vod=vod+1
	S VO(vod)=$char(vcy)_$char(vcx)_$char(vl)_$char(0)_"00000000"_vtyp_$SELECT(vsec:"",1:V)
	;
	I vpos S vpos=124_vpos ; Force Delimiter
	S %TAB(%MAX)=$char((vcy-1))_$char((vcx-1))_$char(vl)_$SELECT($get(vprot):2,1:vsec)_+vreq_vtyp_vpos_"|*"_vlvn_"|"_$S(vnam="":"[*]@"_vlvn,1:vnam)_"|"_vtbl_"|"_vptn_"|"_vxpp_"|"_vxpr_"|"_vmin_"|"_vmax_"|"_vdec_"||"_vlen
	S %MAX=%MAX+1
	Q 
	;
	; I18N=ON
...
GETCA()	;       Return right justified cursor address
	...
	S %TAB(tab)=$E((z),1,2)_($char(vlen))_$E((z),4,999)
	S VO(vod)=$E(VO(vod),1,2)_$char(vlen)_$E(VO(vod),4,999)
	Q 
	;
	;----------------------------------------------------------------------
TEXTONLY	; Message (Protected field)
	...
	;
	S vop=vop+1 S VO(vop)=$char(y)_$char(x)_$char($L(P))_$char(vreq)_"00000001T"_P
	Q 
	;
	;----------------------------------------------------------------------
DECOD(z)	; Decode procedures defined after # (P2 = req, P3 = mode (S=secret)
	;----------------------------------------------------------------------
	;
	S parmlist=parmlist_$SELECT($E(z,1):"/REQ",1:"/NOREQ")
...
IO(prompt)	;Private//Is prompt for output device?
	...
	;
vSIG()	;
	Q "60764^48607^Sudanthiran S. Kumar^20262" ; Signature - LTD^TIME^USER^SIZE
	;
vOpen1()	;	PROMPT FROM STBLPROMPT WHERE PROMPTID='MISSING'
	...
	;
vL1a0	S vos1=0 Q
vL1a1	I '($D(^STBL("PROMPT","MISSING"))#2) G vL1a0
	Q
	;
vFetch1()	;
	;
...
vOpen2()	;	PROMPT FROM STBLPROMPT WHERE PROMPTID=:P
	...
	;
vL2a0	S vos3=0 Q
vL2a1	S vos4=$G(P) I vos4="" G vL2a0
	I '($D(^STBL("PROMPT",vos4))#2) G vL2a0
	Q
	;
vFetch2()	;
	;
