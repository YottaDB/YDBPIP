...
UID	; Standard PROFILE username access
	...
	;
	D GETPWD(.scau)
	I ER N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" D ^SCAUFILE(scau,"/CASDEL/INDEX/JOURNAL/LOG/TRIGAFT/TRIGBEF/UPDATE/VALDD/VALFK/VALREQ/VALRI/VALST/",1) K vobj(scau,-100) S vobj(scau,-2)=1 Tcommit:vTp   D EXIT K vobj(+$G(scau)) Q 
	I $get(%fkey)="TIM" N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" D ^SCAUFILE(scau,"/CASDEL/INDEX/JOURNAL/LOG/TRIGAFT/TRIGBEF/UPDATE/VALDD/VALFK/VALREQ/VALRI/VALST/",1) K vobj(scau,-100) S vobj(scau,-2)=1 Tcommit:vTp   D EXIT K vobj(+$G(scau)) Q 
	;
...
ATTACH(pid,fn,input)	;
	...
	D INIT
	;
	N tmp,vop1 S tmp=$$vDb10(pid,.vop1)
	;
	...
	S vobj(savedrv,1,1)=""
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" D vReSav1(savedrv) S vobj(savedrv,-2)=1 Tcommit:vTp  
	;
	; Load menu data
	N tmpmenu S tmpmenu=$$vDb4(pid,1)
	S MENU=1
	...
	S vop3=JOB
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^TMP(0,vop3,vop2)=$$RTBAR^%ZFUNC(newmenu) S vop4=1 Tcommit:vTp  
	;
...
STFENABL(%fn)	;
	N vret
	;
	N scatbl,vop1 S scatbl=$$vDb12(%fn,.vop1)
	I '$G(vop1) Q 0
	;
...
PAINTSCR	; Paint header, copyright, and message
	...
	;
	 WRITE $$CUP^%TRMVT(11,7),$$DBLH^%TRMVT("Profile Framework")
	;
...
VALIDUID	; Validate %UID and device restrictions
	...
	N device,vop1 S vop1=TLO,device=$$vDb13(TLO)
...
VARSET(scau)	;
	...
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" D ^SVDRVFL(savedrv,"/CASDEL/INDEX/JOURNAL/LOG/TRIGAFT/TRIGBEF/UPDATE/VALDD/VALFK/VALREQ/VALRI/VALST/",1) K vobj(savedrv,-100) S vobj(savedrv,-2)=1 Tcommit:vTp  
	;
	; Load menu data
	S MENU=1
	;
	...
	;
	; Save menu data
	N tmpmenu,vop1,vop2,vop3 S vop2=JOB,vop1=1,tmpmenu=$$vDb14(JOB,1,.vop3)
	S $P(tmpmenu,$C(124),1)=MNUM
	S $P(tmpmenu,$C(124),2)=MPRMPT
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^TMP(0,vop2,vop1)=$$RTBAR^%ZFUNC(tmpmenu) S vop3=1 Tcommit:vTp  
	;
	K vobj(+$G(savedrv)) Q 
	;
...
CHKSTAT(scau)	; Check User STATUS
	...
	;
vSIG()	;
	Q "60723^27201^SSethy^17845" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vDb1(v1)	;	vobj()=Db.getRecord(SCAU,,1)
	...
	Q vOid
	;
vDb10(v1,v2out)	;	voXN = Db.getRecord(TMP,,1,-2)
	;
	N tmp
	S tmp=$G(^TMP(0,v1))
	I tmp="",'$D(^TMP(0,v1))
	...
	Q tmp
	;
vDb11()	;	voXN = Db.getRecord(DDPSTS,,0)
	;
	I '$D(^%ZDDP)
	I $T S $ZS="-1,"_$ZPOS_",%PSL-E-RECNOFL,,DDPSTS" X $ZT
	Q ""
	;
vDb12(v1,v2out)	;	voXN = Db.getRecord(SCATBL,,1,-2)
	;
	N scatbl
	...
	Q scatbl
	;
vDb13(v1)	;	voXN = Db.getRecord(DEVICE,,1)
	;
	I '$D(^%SYSVAR(3,v1))
	;
	Q ""
	;
vDb14(v1,v2,v2out)	;	voXN = Db.getRecord(TMPMENU,,1,-2)
	;
	N tmpmenu
	...
	Q tmpmenu
	;
vDb3(v1)	;	vobj()=Db.getRecord(SAVEDRV,,1)
	;
	N vOid
	...
	Q vOid
	;
vDb4(v1,v2)	;	vobj()=Db.getRecord(TMPMENU,,1)
	;
	N vOid
	S vOid=$O(vobj(""),-1)+1,vobj(vOid,-1)="RecordTMPMENU"
	S vobj(vOid)=$G(^TMP(0,v1,v2))
	I vobj(vOid)="",'$D(^TMP(0,v1,v2))
	S vobj(vOid,-2)='$T
	;
	S vobj(vOid,-3)=v1
	S vobj(vOid,-4)=v2
...
vL1a0	S vos1=0 Q
vL1a1	S vos2=""
vL1a2	S vos2=$O(^CTBL("CTBLUDFN",vos2),1) I vos2="" G vL1a0
	Q
	;
vFetch1()	;
	;
...
vReCp1(v1)	;	RecordTMPMENU.copy: TMPMENU
	...
	Q vobj(v1)
	;
vReSav1(savedrv)	;	RecordSAVEDRV saveNoFiler()
	;
	S ^TMP(0,"SAVEDRV",vobj(savedrv,-3))=$$RTBAR^%ZFUNC($G(vobj(savedrv)))
	N vC,vS s vS=0
	F vC=1:450:$L($G(vobj(savedrv,1,1))) S vS=vS+1,^TMP(0,"SAVEDRV",vobj(savedrv,-3),vS)=$E(vobj(savedrv,1,1),vC,vC+449)
	Q
	;
vtrap1	;	Error trap
	;
