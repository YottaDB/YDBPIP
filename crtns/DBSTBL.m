...
MAIN(vREF,vINFO,vPT,vPB)	;
	...
	.	;
	.	N visDone2
	.	N voffset N vpagenum N vpagenum N vrdsp N vreccnt N vrecs N vrownum N vrows N vrptr N vrr
	.	N vr
	.	;
	.	N vrs,exe,sqlcur,vd,vi,vsql,vsub
	.	;
	.	;   #ACCEPT Date=11/29/04;PGM=RussellDS;CR=13258
	.	I vINFO("TYPE")="Select" S vrs=$$vOpen0(.exe,.vsql,vSELECT,vFROM,vWHERE,vORDERBY,"","/DQMODE=1")
	.	;
	.	S (vpagenum,vrptr)=1
...
PICK(vrs,vINFO,vr,vREF,value)	;
	...
	....				I ($$vStrUC($E(vr(I),1))=vchar) S hit=1
	....				Q 
	...			I hit S vrownum=I
	...			Q 
	..		;
...
HLP(vrs,vINFO,vr,vREF)	;
	...
	Q:(key="")  ; Invalid
	;
	N rs,vos1,vos2,vos3,vos4,vos5  N V1 S V1=key S rs=$$vOpen1()
	;
	I '$G(vos1) D  Q 
...
SETUP(vREF,vINFO)	;
	...
	S vcol=1
	;
	I vINFO("FMT")="U" S vINFO("STR")=$$vStrUC(vINFO("STR"))
	;
	I (vtype="@") D  Q:ER!(vREF="") 
...
DINAM(vREF,vINFO)	;
	...
	..		S LASTTBL=TABLEREF
	..		;
	..		N dbtbl1d S dbtbl1d=$$vDb4("SYSDEV",TABLEREF,NAME)
	..		;
...
PARSE(params,VALCMDS,vINFO)	;
	...
	.	S NAME=$$vStrUC($piece(CMDSTRING," ",1))
...
isCMD(CMD,TEST)	;
	...
	;
UACN(REF,SELECT,TABLES,COLS,HDG)	;
	;
	N I
	N vINFO
	;
...
LOOKUPPP(FORMAT,FILES,ARRAY)	;
	...
	;
vSIG()	;
	Q "60738^38366^Dan Russell^40964" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vOpen1()	;	SEQ,DOC FROM DBTBL12 WHERE %LIBS='SYSDEV' AND FID=:HLPTBL AND CODE=:V1 ORDER BY SEQ ASC
	...
	;
vL1a0	S vos1=0 Q
vL1a1	S vos2=$G(HLPTBL) I vos2="" G vL1a0
	S vos3=$G(V1) I vos3="" G vL1a0
	S vos4=""
vL1a4	S vos4=$O(^DBTBL("SYSDEV",12,vos2,vos3,vos4),1) I vos4="" G vL1a0
	Q
	;
vFetch1()	;
	;
	...
	I vos1=0 Q 0
	;
	S vos5=$G(^DBTBL("SYSDEV",12,vos2,vos3,vos4))
	S rs=$S(vos4=$C(254):"",1:vos4)_$C(9)_$P(vos5,"|",1)
	;
	Q 1
	;
vtrap1	;	Error trap
	...
	S value=""
	;
	I ($P(error,",",3)["%GTM") S $ZS=error X voZT
	Q 
