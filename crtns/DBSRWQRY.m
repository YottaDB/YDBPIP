DBSRWQRY(dbtbl5h,PQINFO,WHERE)	;
	...
	; Load raw query
	;
	N ds,vos1,vos2,vos3,vos4,vOid S ds=$$vOpen1()
	;
	...
	.	N X
	.	;
	.	N dbtbl5pr S dbtbl5pr=$G(^DBTBL($P(ds,$C(9),1),5,$P(ds,$C(9),2),$P(ds,$C(9),3)))
	.	;
...
BLDTAB(WCTYPE,DI,OP,PARAMS,TAB)	;
	...
	.	N dbctlqry S dbctlqry=$$vDb3(0,OP)
...
KEYWRDS(X,TAB)	; Private - Parse X for keywords and build TAB
	...
	;
vSIG()	;
	Q "60680^47183^Dan Russell^11533" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vStrUC(vObj)	; String.upperCase
	...
	;
vDb3(v1,v2)	;	voXN = Db.getRecord(DBCTLQRY,,0)
	;
	N dbctlqry
	S dbctlqry=$G(^DBCTL("QWRD",v1,v2))
	I dbctlqry="",'$D(^DBCTL("QWRD",v1,v2))
	I $T S $ZS="-1,"_$ZPOS_",%PSL-E-RECNOFL,,DBCTLQRY" X $ZT
	Q dbctlqry
	;
...
vL1a1	S vos2=$G(RID) I vos2="" G vL1a0
	S vos3=""
vL1a3	S vos3=$O(^DBTBL(vos3),1) I vos3="" G vL1a10
	S vos4=30.999
vL1a5	S vos4=$O(^DBTBL(vos3,5,vos2,vos4),1) I vos4=""!("50"']]vos4) G vL1a3
	I '(vos4]]"30.999") G vL1a5
	S vd=$S(vos3=$C(254):"",1:vos3)_$C(9)_vos2_$C(9)_$S(vos4=$C(254):"",1:vos4)
