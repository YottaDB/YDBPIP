UCREC4OP	;
	...
	;
	; ---------------------------------------------------------------------
accByDec(decPos,subscr,expr,bAudit,nAssign,bHasRoot)	;
	;
	...
	;
	; ---------------------------------------------------------------------
accByOvs(sr,ovs,expr,bAudit,nAssign,bHasRoot)	;
...
clnAsn1(dec,acc,value)	;
	...
	;
	Q $$wrap^UCREC4OP(acc_"="_value)
	;
	; ---------------------------------------------------------------------
clnByCln(dec,cdouter,cdinner)	;
	;
	N acc S acc=""
	...
	;
	; ---------------------------------------------------------------------
clnByOvs(sr,ovs,cd,bAudit,bAssign,bHasRoot)	;
	;
	N nam S nam=$piece(ovs,"(")
...
decByOvs(sr,ovs)	;
	;
	N ins S ins=$$getAtt^UCGM(ovs,,1)
	N scope S scope=$$getAtt^UCGM(ovs,,3)
	S $piece(ins,$C(9),3)=scope
	S $piece(ins,$C(9),10)=''$$getAtt^UCGM(ovs,,10)
	;
	I (",FORMAL,LITERAL,PUBLIC,"[(","_scope_",")) S $piece(ins,$C(9),10)=1
	;
...
findDec(sr,ovs)	;
	;
	N dp S dp=$$getAtt^UCGM(ovs,,2)+1
	;
	S ovs=$piece(ovs,"(")
	;
	I $order(dbAcc(sr,ovs,""))="" N vo2 S vo2="%PSL-E-INTERNAL no declaration found for "_ovs_" in "_sr,$ZS=($L($P(vo2,","),"-")=3*-1)_","_$ZPOS_","_vo2 X $ZT
	;
	N found S found=0
	F  Q:'(found'!(dp=""))  D
	...
	.	I (dp="") N vo3 S vo3="%PSL-INTERNAL no declaration found for "_ovs_" in "_sr,$ZS=($L($P(vo3,","),"-")=3*-1)_","_$ZPOS_","_vo3 X $ZT
	.	;
	.	N data S data=dbAcc(sr,ovs,dp)
	.	;
	.	I '$$isDoScope($piece(data,$C(9),7),$$getDo^UCPSLST(pslSt)) Q 
	...
	;
	; ---------------------------------------------------------------------
forceMode(sr,ovs,mode)	;
	;
	I $$getAtt^UCGM(ovs,,3)="NEW" D ERROR^UCGM("Method can not be applied to local scope object: "_ovs) Q 
	I $$getAtt^UCGM(ovs,,3)="LITERAL" D ERROR^UCGM("Method can not be applied to literal scope object: "_ovs) Q 
	;
	N dummy S dummy=$$vPSLopti() ; side effect: set optimize
	N expr S expr=$SELECT(mode=0:"Class.new()",1:".getRecord(,,0)")
...
insByDec(decPos,subscr,expr,imd)	;
	...
	;
	N ins S ins=cls
	N scope S scope=$$getAtt^UCGM(nam,,3)
	S $piece(ins,$C(9),2)=decPos
	S $piece(ins,$C(9),3)=scope
	S $piece(ins,$C(9),5)=expr
	;
	...
	..		S $piece(dbAcc(sr,nam,ap),$C(9),4)=$SELECT((insLst=""):insPos,1:insPos_","_insLst)
	..		Q 
	.	Q 
	;
...
optNload(mp1,bOpti,ldMode)	;
	...
	I ldMode=0 S ldMode=$S($$rtIsRdb^UCXDD():2,1:1)
...
paNeedVobj(srn,nam,decLine)	;
	...
	;
	; ---------------------------------------------------------------------
purByDec(decPos,subscr,pur,bHasRoot)	;
	;
	N decOvs S decOvs=dbAcc("*","*",decPos)
	N sr S sr=$piece(decOvs,$C(9))
	N nam S nam=$piece(decOvs,$C(9),2)
...
setAssign(sr,ovs,assign)	;
	...
	;
	; Negative values cannot be changed
	I cur<0,cur'=assign S $ZS="-1,"_$ZPOS_","_"%PSL-E-RECMODE, conflicting assignment modes for "_ovs X $ZT
	;
	I assign>cur!(cur="") D
...
wrap(value)	; position
	...
	;
	; ---------------------------------------------------------------------
wrapFits(code,reserve)	;
	;
	Q ($L((((code))))+($L(code,$C(7))*10+reserve)'>1980)
	;
	; ---------------------------------------------------------------------
clnAsn2(mp1)	; pass 1 code
	;
	N eq S eq=$F(mp1,"=")
	...
	;
	N ovs S ovs=$SELECT((subs=""):dec,1:nam_subs)
	;
	N cd S cd=$$caPslCln^UCXDD(.pslCln,prop,.pslTbl)
	;
	I $piece(dbAcc(srn,nam,dec),$C(9),11)=1 S mode=mode+3
	;
	N code S code=$$getUpdCode^UCXDD(cd,ovs,$E(mp1,eq,1048575),mode)
	;
	I code[$char(9) D
	.	I ($L((code))'>1980) S code=$$vStrRep(code,$char(9),"",0,0,"") Q 
...
createPos(line)	; Target pass 1 line   /NOREQ
	;
	N val S val=$SELECT(($D(line)#2):line,1:$$getDcLnr^UCGM())
	;
...
decOpt(dec,mp1)	;
	...
	;
	N td S td=$$caPslTbl^UCXDD(.pslTbl,$$getReTable^UCGM($piece(decDes,$C(9))),0)
	;
	N i ; iterator
	...
	I '(nodInc="") D
	.	N dummy
	.	I ($P(td,"|",8)'="GTM"),($P(td,"|",5)["B")!($P(td,"|",5)["M") D
	..		D decOptLv(dec,-151,srn,.lvnLst)
	..		Q 
	.	;
	.	N pk S pk=$P(td,"|",3)
	.	;
	.	F i=1:1:$S((pk=""):0,1:$L(pk,",")) D
	..		D decOptLv(dec,-2-i,srn,.lvnLst)
	..		Q  
	.	Q  
	;
	...
	.	I '($D(dbAcc("*",dec,-151))#2) D decOptLv(dec,-151,srn,.lvnLst)
	.	S kilLst=$S((kilLst=""):dbAcc("*",dec,-151),1:kilLst_","_dbAcc("*",dec,-151))
	.	Q 
	S $piece(decDes,$C(9),15)=kilLst S dbAcc(srn,ovs,dec)=decDes
	;
	I '(lvnLst="") S mp1(dec\1)=$$decNew(ovs,mp1(dec\1),lvnLst)
	;
	...
	.	I $$isRecord^UCGM(insCls) S insCod=$$insOptCp(td,srn,ovs,ins,insDes,lbl,.apl,head,tail)
...
decSav(dec,lvmap)	;
	...
	.	I '(code="") S code=$C(9)
	.	S code=" D VOBJ^DBSDBASE("_ovn_code_",$C("_$P(td,"|",10)_"))"
	...
	.	S:($D(dbAcc("*",dec,-151))#2) reserve=$S(($P(td,"|",3)=""):0,1:$L($P(td,"|",3),","))*60+reserve
...
insApa2Apl(apa)	; actual parameter array
	...
	;
	; ---------------------------------------------------------------------
insCode(mp1,ovs,occ,lbl,apl,head,tail)	;
	;
	N del S del="S "_ovs_"="
	N exp
	N apn N ptr
...
insOptAP(decPos,apl)	;
	...
	;
	; ---------------------------------------------------------------------
insOptCa(td,srn,ovs,ins,insDes,lbl,apl,head,tail)	;
	;
	...
	N vdbLbl ; label of $$vDbN() in append(,)
	;
	F  S lnr=$order(append(lbl,lnr)) Q:lnr=""  D
	.	I append(lbl,lnr)'["$$vDb" Q 
	.	D insCode(" S vOid=$$vDb"_$piece(append(lbl,lnr),"$$vDb",2),"vOid",1,.vdbLbl,.vdbApl)
	.	;
	.	S vdbDes=insDes
	...
	;
	I '$D(labels(p2lbl)) D CacheSr^UCDB(td,0,p2lbl,cmt,expr,dflt,vdbLbl)
	;
	...
	;
	; ---------------------------------------------------------------------
insOptCl(td,srn,ovs,ins,insDes,lbl,apl,head,tail)	;
	;
	D decrLabel^UCGM(lbl) ; decrement reference count old label
	;
	N dec S dec=$piece(insDes,$C(9),2)
	...
	;
	; ---------------------------------------------------------------------
insOptCp(td,srn,ovs,ins,insDes,lbl,apl,head,tail)	;
	;
	...
	..		I cod'[fnd Q 
	..		;
	..		I cod["S vOid=" S cod=" ;" Q 
	..		I cod[("S "_fnd_")=") S cod=" ;" Q 
	..		;
	..		I cod[(fnd_",-") D
	...			;
	...
	;
	; ---------------------------------------------------------------------
insOptDb(td,srn,ovs,ins,insDes,lbl,apl,head,tail)	;
	;
	I $E(lbl,1,3)'="vDb" D pass2Err(ins,"Unexpected label '"_lbl_"' for Db.getRecord()") Q 
	;
	D decrLabel^UCGM(lbl) ; decrement reference count old label
	;
	N expr S expr=$piece(insDes,$C(9),5)
	...
	S mcode=mcode_"S "
	;
	N cmt S cmt="voXN = Db.getRecord("_$P(td,"|",1)_",,"_dflt
	N decPos S decPos=$piece(insDes,$C(9),2)
	N nod S nod=$order(dbAcc("*",decPos,""))
	N xnods S xnods=""
	;
	I dflt=1,nod'<0,'$$hasPSLSetting^UCGM("DEBUG","DBIOCOUNT") S mcode=mcode_ovs_"="_$$insOptNoVDB(td,decPos,.apl)
	E  D
	.	F i=-2,-99,-151 I ($D(dbAcc("*",decPos,i))#2) S xnods=xnods_",."_dbAcc("*",decPos,i) S cmt=cmt_","_i
	.	S cmt=cmt_")"
	.	I ($P(td,"|",3)="") S xnods=$E(xnods,2,1048575)
	.	;
	.	N p2lbl S p2lbl=$$findSubr^UCGM("vDb",cmt)
	.	I '$D(labels(p2lbl)) D
	..		D getRecSr^UCDB(td,decPos,p2lbl,cmt,dflt)
	..		Q 
	.	S mcode=mcode_$$insOptAP(decPos,.apl)_ovs_"=$$"_p2lbl_"("_$$insApa2Apl(.apl)_xnods_")"
	.	Q 
	;
	Q mcode_tail
	;
	; ---------------------------------------------------------------------
insOptDs(td,srn,ovs,ins,insDes,lbl,apl,head,tail)	;
	;
	N decPos S decPos=$piece(insDes,$C(9),2)
	;
	I ($D(dbAcc("*",decPos,-151))#2)!$$hasPSLSetting^UCGM("DEBUG","DBIOCOUNT") Q $$insOptDb(td,srn,ovs,ins,insDes,lbl,.apl,head,tail)
	I ($D(dbAcc("*",decPos,-2))#2) S tail=","_dbAcc("*",decPos,-2)_"=1"_tail
	;
	I $E(lbl,1,3)'="vDb" D pass2Err(ins,"Unexpected label '"_lbl_"' for DbSet.getRecord()") Q ""
	;
	D decrLabel^UCGM(lbl) ; decrement reference count old label
	;
	Q head_"S "_$$insOptAP(decPos,.apl)_ovs_"="_$$insOptNoVDB(td,decPos,.apl)_tail
	;
	; ---------------------------------------------------------------------
insOptNoVDB(td,decPos,apl)	;
	N vret
	N i
	N lvpm
...
isAutoLoad(purPos)	; Purpose identifier
	...
	;
	N tbl S tbl=$$getReTable^UCGM($piece(data,$C(9)))
	;
...
ldMap(ldMode,ldMap)	;
	...
	.	N td S td=$$caPslTbl^UCXDD(.pslTbl,$$getReTable^UCGM($piece(des,$C(9))),1)
	.	;
	.	I '($$getArchiveTable^DBARCHIVE(td)="") S expr=$$lvpm(dec,-99,1)
	.	;
	.	I '$$rtIsRdb^UCXDD() Q  ; no extras for MDB inc load
	.	;
	.	I '($P(td,"|",8)'="GTM") Q  ; MDB table in RDB environment
	.	;
	.	I $piece(ldMap(pos),$C(9))=-1 S expr=$$lvpm(dec,-2,1)
	.	I ($P(td,"|",5)["B")!($P(td,"|",5)["M") S expr=$$lvpm(dec,-2,1) S expr=$$lvpm(dec,-151,1) Q 
	.	Q 
	;
	Q 
	;
	; ---------------------------------------------------------------------
ldMap1Acc(ldMap,load4,pur)	;
	;
	...
	..		;
	..		S (rmd,exs,cc,cci,ins)=""
	..		F  Q:'((rmd'=-2))  S ins=$order(load4(pur,stk,acc,ins)) Q:ins=""  D ins2me(ins,.rmd,.exs,.cc) I (cci=""),'(cc="") S cci=ins
	..		;
...
ldMap1Ins(ldMap,load4,ins)	;
	...
	;
	; ---------------------------------------------------------------------
ldMap4Acc(purByDo,pur,acc,accDo,insList)	;
	;
	...
	;
	; ---------------------------------------------------------------------
ldMap4Ins(insByPur,pur,acc,accDo,insList)	;
	;
	N elm
	F elm=1:1:$S((insList=""):0,1:$L(insList,",")) D
	.	N ins S ins=$piece(insList,",",elm)
...
loadAcc(mp1,acc,occDes,pp2lv)	;
	...
	N lvpm
	;
	I '($piece(accDes,$C(9),16)="") D loadLvpm(.lvpm,dec,$piece(accDes,$C(9),16))
	;
	N cod
	N elm
	...
	.	I ($D(dbAcc("*",dec,-99))#2) S cod=dbAcc("*",dec,-99) S lvpm(-99)=$SELECT($$isWrap(cod):pp2lv($$stripWrap(cod)),1:cod)
...
loadCmp(dec,pur,rmd,exs,lvpm)	;
	...
	;
	; ---------------------------------------------------------------------
loadCode(dec,pur,rmd,exs,cacheIns,lvpm)	;
	;
	...
	N tbl S tbl=$$getReTable^UCGM($piece(dbAcc(srn,ovs,dec),$C(9)))
	;
	I '($D(pslTbl(tbl))#2) S pslTbl(tbl)=$$getPslTbl^UCXDD(tbl,0)
	;
	I $$nodIsBM^UCXDD(node)!$$nodIsNeg^UCXDD(node)!($P(pslTbl(tbl),"|",8)'="GTM") S cacheIns=""
	I $E(node,1)="~" S code=$$loadCmp(dec,pur,rmd,exs,.lvpm)
	E  I (cacheIns="") S code=$$getLodCode^UCXDD(pslTbl(tbl),dec,node,rmd,exs,.lvpm)
	E  D
...
loadIns(mp1,ins,occDes,pp2lv)	;
	...
	N lvpm
	;
	I '($piece(insDes,$C(9),16)="") D loadLvpm(.lvpm,dec,$piece(insDes,$C(9),16))
	;
	N lnr S lnr=(ins\1)+1
	;
	N ln1 S ln1=$order(mp1(ins\1))
	N needSrn S needSrn=$E($translate(mp1(ln1),$C(9,32)),1,lvl+1)?."."
	;
	I needSrn D
...
loadLvpm(lvpm,dec,subs)	;
	...
	S tbl=$$getReTable^UCGM($piece(des,$C(9)))
	...
	.	Q 
	;
	F n=-2,-151 I '($D(lvpm(n))#2) S lvpm(n)=$$lvpm^UCXDD(nam,n)
...
lvMap(map)	; purPos to lvn map   /MECH=REFARR:W
	...
	...			N cd S cd=$$caPslCln^UCXDD(.pslCln,tbl_$translate(nod,"*","."),.pslTbl)
	...			S nodLvpm=$$getCurNode^UCXDD(cd,0)
	...			; may need to ensure that -151 node is defined ???
	...			Q 
	..		E  S nodLvpm=nod
	..		;
...
pur2M(code,map,override)	;
	...
	..		I $$vStrIsNum($piece(wrap,"=")) S wrap=$$clnAsn2(wrap)
	..		Q 
	.	;
...
setDecVal(sr,ovs,field,val)	;
	...
	;
vSIG()	;
	Q "60801^36052^Frans S.C. Witte^167184" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
