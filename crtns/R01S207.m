R01S207	; DBINDX - DATA-QWIK Data Dictionary X-REF Report
	;
	; **** Routine compiled from DATA-QWIK Report DBINDX ****
	;
	...
	;
	S RID="DBINDX"
	S RN="DATA-QWIK Data Dictionary X-REF Report"
	I $get(IO)="" S IO=$I
	;
	D INIT^%ZM()
	;
	S %TAB("IO")=$$IO^SCATAB
	S %TAB("VIN3")="|255||[DBINDX]FID|[DBTBL1]||S FID=X D EXT^DBSQRY||T|File Name|||||"
	S %TAB("VIN4")="|255||[DBINDX]DINAM|[DBINDX]DINAM:DISTINCT:NOVAL||I ""ALL""[FID S I(3)="""" D EXT^DBSQRY||T|Data Item|||||"
	;
	S %READ="IO/REQ,VIN3#0,VIN4#0,"
	;
	; Skip device prompt option
	I $get(VRWOPT("NOOPEN")) S %READ="VIN3#0,VIN4#0,"
	;
	S VFMQ=""
...
V0	; External report entry point
	...
	N VWHERE
	N %TIM N CONAM N RID N RN N VL N VLOF N VRF N VSTATS N ZNAME N vCOL N vHDG N vc1 N vc2 N vc3 N vc4 N vc5 N vc6 N vc7 N vc8 N vc9 N vovc1 N vovc2 N vovc3 N vovc4 N vovc5 N vrundate N vsysdate
	;
	N cuvar S cuvar=$$vDb2()
	 S cuvar=$G(^CUVAR("BANNER"))
	;
	S CONAM="DATA-QWIK 7.2"
	S ER=0 S RID="DBINDX" S RN="DATA-QWIK Data Dictionary X-REF Report"
	S VL=""
	;
	...
	..		N CHRSET S CHRSET=$$^UCIOENCD("Report","DBINDX","V0","*")
	...
	D INIT^%ZM()
	;
	S vCOL="[DBINDX]INDEXID#23#17,[DBINDX]IDNAME#41#40"
	;
	; Build WHERE clause to use for dynamic query
	D
	.	N SEQ S SEQ=1
	.	N DQQRY N FROM
	.	S DQQRY(1)="[DBINDX]LIBS = ""SYSDEV""" S SEQ=SEQ+1
	.	S DQQRY(2)="[DBINDX]DBOPT=2,3,4,5,6,7,8,9,13" S SEQ=SEQ+1
	.	I $get(VIN3)="" S VIN3="ALL"
	.	I VIN3'="ALL" S DQQRY(3)="[DBINDX]FID "_VIN3 S SEQ=SEQ+1
	.	I $get(VIN4)="" S VIN4="ALL"
	.	I VIN4'="ALL" S DQQRY(SEQ)="[DBINDX]DINAM "_VIN4 S SEQ=SEQ+1
	.	S FROM=$$DQJOIN^SQLCONV("DBINDX") Q:ER 
	...
	.	N VBNRINFO
	.	;
	.	S VBNRINFO("PROMPTS",3)="WC2|"_"File Name"_"|VIN3|"_$get(VIN3)
	.	S VBNRINFO("PROMPTS",4)="WC2|"_"Data Item"_"|VIN4|"_$get(VIN4)
	.	;
	.	D
	..		N SEQ
	..		N VALUE N VAR N X
	...
	..		Q 
	.	;
	.	S VBNRINFO("DESC")="DATA-QWIK Data Dictionary X-REF Report"
	.	S VBNRINFO("PGM")="R01S207"
	.	S VBNRINFO("RID")="DBINDX"
	.	S VBNRINFO("TABLES")="DBINDX"
	.	;
	.	S VBNRINFO("ORDERBY",1)="[SYSDEV,DBINDX]LIBS"
	.	S VBNRINFO("ORDERBY",2)="[SYSDEV,DBINDX]DBOPT"
	.	S VBNRINFO("ORDERBY",3)="[SYSDEV,DBINDX]FID"
	.	S VBNRINFO("ORDERBY",4)="[SYSDEV,DBINDX]DINAM"
	.	S VBNRINFO("ORDERBY",5)="[SYSDEV,DBINDX]INDEXID"
	.	;
	...
	N rwrs,vos1,vos2,sqlcur,exe,sqlcur,vd,vi,vsql,vsub S rwrs=$$vOpen0(.exe,.vsql,"DBINDX.LIBS,DBINDX.DBOPT,DBINDX.FID,DBINDX.DINAM,DBINDX.INDEXID,DBINDX.FLDESC,DBINDX.DIDESC,DBINDX.IDNAME,DBINDX.OPTDESC","DBINDX",VWHERE,"DBINDX.LIBS,DBINDX.DBOPT,DBINDX.FID,DBINDX.DINAM,DBINDX.INDEXID","","/DQMODE=1")
...
VSAVLAST	; Save last access keys values
	...
	;
VGETDATA(V,VI)	;
	S vc1=$piece(V,$char(9),1) ; DBINDX.LIBS
	S vc2=$piece(V,$char(9),2) ; DBINDX.DBOPT
	S vc3=$piece(V,$char(9),3) ; DBINDX.FID
	S vc4=$piece(V,$char(9),4) ; DBINDX.DINAM
	S vc5=$piece(V,$char(9),5) ; DBINDX.INDEXID
	S vc6=$piece(V,$char(9),6) ; DBINDX.FLDESC
	S vc7=$piece(V,$char(9),7) ; DBINDX.DIDESC
	S vc8=$piece(V,$char(9),8) ; DBINDX.IDNAME
	S vc9=$piece(V,$char(9),9) ; DBINDX.OPTDESC
	Q 
	;
...
VBREAK	;
	...
	I 0!(vovc1'=vc1) S vs(2)=0 S vh(2)=0 S VD(1)=0 S vb2=1 S vb3=1 S vb4=1 S vb5=1 S VH0=1
	I vb2!(+vovc2'=+vc2) S vs(3)=0 S vh(3)=0 S VD(2)=0 S vb3=1 S vb4=1 S vb5=1 S VH0=1
	I vb3!(vovc3'=vc3) S vs(4)=0 S vh(4)=0 S VD(3)=0 S vb4=1 S vb5=1
	I vb4!(vovc4'=vc4) S vs(5)=0 S vh(5)=0 S VD(4)=0 S vb5=1
	Q 
	;
...
VHDG4	; Group Header
	...
	D VOM
	S VL=$E(vc3,1,20)
	S VL=VL_$J("",22-$L(VL))_$E(vc6,1,40)
	D VOM
	S VL="" D VOM
	Q 
	;
VHDG5	; Group Header
	...
	S VL="" D VOM
	S VL="    "_$E(vc4,1,20)
	S VL=VL_$J("",26-$L(VL))_$E(vc7,1,40)
	D VOM
	S VL="" D VOM
	Q 
	;
VDTL5	; Detail
	...
	;
	S VL="                      "_$E(vc5,1,17)
	S VL=VL_$J("",40-$L(VL))_$E(vc8,1,40)
	D VOM
	Q 
	;
VHDG0	; Page Header
	...
	S VL=VL_$J("",48-$L(VL))_"Run Date:"
	S VL=VL_$J("",58-$L(VL))_$E(vrundate,1,10)
	S VL=VL_$J("",70-$L(VL))_$E(%TIM,1,8)
	D VOM
	S VL=RN_"  ("_RID_")"
	S VL=VL_$J("",50-$L(VL))_"System:"
	S VL=VL_$J("",58-$L(VL))_$E(vsysdate,1,10)
	S VL=VL_$J("",70-$L(VL))_"Page:"
	S VL=VL_$J("",76-$L(VL))_$E(($J(VPN,3)),1,3)
	D VOM
	S VL="" D VOM
	D VP1 Q:VFMQ!$get(verror)  S V=$E(ZNAME,1,40) S VL=V
	D VOM
	S VL="" D VOM
	S VL="File Name  Data Item"
	D VP2 Q:VFMQ!$get(verror)  S V=$E(ZNAME,1,12)
	S VL=VL_$J("",22-$L(VL))_V
	S VL=VL_$J("",40-$L(VL))_"Description"
	D VOM
	S VL="================================================================================"
	D VOM
	;
