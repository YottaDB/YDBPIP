...
RsMsSel(bld,map,sqllit,mode,clause,kwds)	;
	...
	...			Q 
	..		E  D
	...			S selnam=$$vStrUC($translate(elem,"*/+-|():""% "_$char(0),"_______"))
...
wrEXECSP(vIndex,vSp,vKeyVals,vKeyNum,vDlm)	;
	...
	;
	I vEr<0 S $ZS="-1,"_$ZPOS_","_"%PSL-E-SQLFAIL,"_$TR($get(vRm),$C(10,44)," ~") X $ZT
	;
	Q vData
	;
	; ---------------------------------------------------------------------
...
wrSELECT(vIndex,vSql,vDlm,vKeyVals)	;
	...
	;
vSIG()	;
	Q "60736^26834^Frans S.C. Witte^101133" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
