DBARCHIVE	;
	...
	Q 
	;
ARCHIVE(ARCHTBL,THRUDATE,SELECT,FROM,WHERE)	;
	;
	N stop
	...
	.	Q 
	;
	LOCK +DBARCHIVE(ARCHTBL):1 E  D  Q ERMSG
	.	;
	...
	.	Q 
	;
	N rsarchist,vos1,vos2,vos3,vos4 S rsarchist=$$vOpen1()
	;
	I $$vFetch1() S archseq=rsarchist+1
	E  S archseq=1
	;
	N dbarchist,vop1,vop2,vop3 S dbarchist="",vop2=ARCHTBL,vop1=archseq,vop3=0
	;
	S $P(dbarchist,$C(124),1)=$P($H,",",1)
	S $P(dbarchist,$C(124),2)=$P($H,",",2)
	S $P(dbarchist,$C(124),5)=0
	S $P(dbarchist,$C(124),6)=0
	S $P(dbarchist,$C(124),7)=0
	S $P(dbarchist,$C(124),8)=THRUDATE
	S $P(dbarchist,$C(124),9)=SELECT
	S $P(dbarchist,$C(124),10)=FROM
	S $P(dbarchist,$C(124),11)=WHERE
	;
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^DBARCHIS(vop2,vop1)=$$RTBAR^%ZFUNC(dbarchist) S vop3=1 Tcommit:vTp  
	;
	I (archkey=1) D
	.	;
	.	N reccnt
	...
	.	;
	. S $P(dbarchist,$C(124),5)=reccnt
	. S $P(dbarchist,$C(124),6)=reccnt
	.	;
	. N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^DBARCHIS(vop2,vop1)=$$RTBAR^%ZFUNC(dbarchist) S vop3=1 Tcommit:vTp  
	.	;
	...
	.	I '($E($$vStrUC(SELECT),1,8)="DISTINCT") S SELECT="DISTINCT "_SELECT
	.	;
	.	; Strip DISTINCT for order by
	.	S ORDERBY=$piece(SELECT," ",2,$L(SELECT))
	.	;
	.	;          #ACCEPT Date=03/12/07; Pgm=RussellDS; CR=25675; Group=Dynamic
	.	N rs,exe,sqlcur,vd,vi,vsql,vsub S rs=$$vOpen0(.exe,.vsql,SELECT,FROM,WHERE,ORDERBY,"","")
	.	;
	.	S (primcnt,reccnt)=0
	...
	.	K vobj(+$G(rs)) Q 
	;
	S $P(dbarchist,$C(124),3)=$P($H,",",1)
	S $P(dbarchist,$C(124),4)=$P($H,",",2)
	;
	I stop S $P(dbarchist,$C(124),7)=1
	;
	N vTp S vTp=0 S:($Tlevel=0) vTp=1 Tstart:vTp (vobj):transactionid="CS" S ^DBARCHIS(vop2,vop1)=$$RTBAR^%ZFUNC(dbarchist) S vop3=1 Tcommit:vTp  
	;
...
getArchiveTable(tblDes)	; table descriptor
	...
	.	;
	.	N dbutarchive,vop1 S vop1=ds,dbutarchive=$G(^UTBL("DBARCHIVE",vop1))
	.	;
	.	N archDes S archDes=$$getPslTbl^UCXDD(vop1,1)
	.	;
	.	; Sub-table
...
getArchivable(tblDes)	; Table descriptor of table to check
	...
	;
	N rs,vos1,vos2,vos3,vos4  N V1 S V1=global S rs=$$vOpen4()
	;
	...
	.	S FID=rs
	.	;
	.	N fidDes S fidDes=$$getPslTbl^UCXDD(rs,1)
	.	;
...
getArchiveKey(tblDes,forArch)	;
	...
	I (archkey=0) D
	.	;
	.	N rs,vos1,vos2,vos3,vos4  N V1 S V1=table S rs=$$vOpen5()
	.	;
...
getArchiveSubs(tblDes)	; Table descriptor of table to check
	...
	S TBL=$P(tblDes,"|",1)
	;
	N tbldbtbl1,vop1,vop2,vop3,vop4 S vop1="SYSDEV",vop2=TBL,tbldbtbl1=$$vDb3("SYSDEV",TBL)
	 S vop4=$G(^DBTBL(vop1,1,vop2,16))
	 S vop3=$G(^DBTBL(vop1,1,vop2,0))
	;
	S tblkeys=($P(vop4,$C(124),1))
	;
	N rs,vos1,vos2,vos3,vos4  N V1 S V1=$P(vop3,$C(124),1) S rs=$$vOpen6()
	;
	F  Q:'($$vFetch6())  D
	.	;
	.	N isSubTbl
	...
	.	S PSUBTBL=rs
	.	;
	.	N psubdbtbl1,vop5,vop6,vop7 S vop5="SYSDEV",vop6=PSUBTBL,psubdbtbl1=$$vDb3("SYSDEV",PSUBTBL)
...
ARCHTBLCHK(ARCHTBL)	; Archive Table Name
	...
	;
	; Table ~p1 must have a filer
	I ($P(tblDes,"|",6)="") Q $$^MSG(6894,ARCHTBL)
	;
	Q ERRMSG
	;
INCLUDEDCHK(ARCHTBL,INCLUDED)	;
	;
	N ERR
	N archkeys
	N i
	...
	S ERRMSG=""
	;
	N archDes S archDes=$$getPslTbl^UCXDD(ARCHTBL,1)
	;
	...
	.	;
	.	N inclDes S inclDes=$$getPslTbl^UCXDD(TBL,1)
	.	;
	...
	;
vSIG()	;
	Q "60736^64685^Dan Russell^22239" ; Signature - LTD^TIME^USER^SIZE
	; ----------------
...
vDb3(v1,v2)	;	voXN = Db.getRecord(DBTBL1,,0)
	;
	N tbldbtbl1
	S tbldbtbl1=$G(^DBTBL(v1,1,v2))
	I tbldbtbl1="",'$D(^DBTBL(v1,1,v2))
...
vL1a1	S vos2=$G(ARCHTBL) I vos2="" G vL1a0
	S vos3=""
vL1a3	S vos3=$O(^DBARCHIS(vos2,vos3),1) I vos3="" G vL1a6
	S vos4=$S('$D(vos4):$S(vos3=$C(254):"",1:vos3),vos4<$S(vos3=$C(254):"",1:vos3):$S(vos3=$C(254):"",1:vos3),1:vos4)
	G vL1a3
...
vL2a0	S vos1=0 Q
vL2a1	S vos2=""
vL2a2	S vos2=$O(^PROCID(vos2),1) I vos2="" G vL2a0
	S vos3=$G(^PROCID(vos2))
	I '($P(vos3,"|",6)="ARCHIVE") G vL2a2
	Q
	;
vFetch2()	;
	;
	...
	I vos1=0 Q 0
	;
	S rs=$S(vos2=$C(254):"",1:vos2)_$C(9)_$P(vos3,"|",7)
	;
	Q 1
	;
...
vL3a0	S vos1=0 Q
vL3a1	S vos2=""
vL3a2	S vos2=$O(^UTBL("DBARCHIVE",vos2),1) I vos2="" G vL3a0
	Q
	;
vFetch3()	;
	;
...
vOpen4()	;	FID FROM DBTBL1 WHERE %LIBS='SYSDEV' AND FID<>:ARCHTBL AND GLOBAL=:V1
	...
	;
vL4a0	S vos1=0 Q
vL4a1	S vos2=$G(ARCHTBL) I vos2="",'$D(ARCHTBL) G vL4a0
	S vos3=$G(V1) I vos3="",'$D(V1) G vL4a0
	S vos4=""
vL4a4	S vos4=$O(^XDBREF("DBTBL1.GLOBAL","SYSDEV",vos3,vos4),1) I vos4="" G vL4a0
	I '(vos4'=vos2) G vL4a4
	Q
	;
vFetch4()	;
...
vOpen6()	;	FID FROM DBTBL1 WHERE %LIBS='SYSDEV' AND GLOBAL=:V1 AND FID<>:TBL
	...
	;
vL6a0	S vos1=0 Q
vL6a1	S vos2=$G(V1) I vos2="",'$D(V1) G vL6a0
	S vos3=$G(TBL) I vos3="",'$D(TBL) G vL6a0
	S vos4=""
vL6a4	S vos4=$O(^XDBREF("DBTBL1.GLOBAL","SYSDEV",vos2,vos4),1) I vos4="" G vL6a0
	I '(vos4'=vos3) G vL6a4
	Q
	;
vFetch6()	;
...
vOpen7()	;	ARCHTBL FROM DBUTARCHIVE WHERE ARCHTBL<>:ARCHTBL
	...
	;
vL7a0	S vos1=0 Q
vL7a1	S vos2=$G(ARCHTBL) I vos2="",'$D(ARCHTBL) G vL7a0
	S vos3=""
vL7a3	S vos3=$O(^UTBL("DBARCHIVE",vos3),1) I vos3="" G vL7a0
	I '(vos3'=vos2) G vL7a3
	Q
	;
vFetch7()	;
