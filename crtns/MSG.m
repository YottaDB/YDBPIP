MSG(MSGID,p1,p2,p3,p4,p5)	;
	...
	..		S vofst=2
	..		;
	..		I vi=3,$E(vmsg,4)=" " S vmsg=$E(vmsg,1,vi)_$E(vmsg,vi+2,999) Q 
	..		;
	..		I $E(vmsg,vi-3)=" ",$E(vmsg,vi+1)=" " S vmsg=$E(vmsg,1,vi)_$E(vmsg,vi+2,999)
	..		Q 
	.	;
	.	S vmsg=$E(vmsg,1,vi-3)_v_$E(vmsg,vi+1,999)
	.	Q 
	;
	S %MSGID=MSGID_"|"_$$V2LV(.v)
	Q vmsg
	;
V2LV(val,fs,opt)	;
	;
	...
	S x=""
	I $D(val)=1 D
	.	I ($get(fs)="") S x=$$LV(val) Q 
	.	F i=1:1:$L(val,fs) S y=$piece(val,fs,i) S x=x_$$LV(y)
	.	Q 
	;
...
LV(val)	;
	...
	.	N i N n
	.	;
	.	F i=1:1 S x(i)=x#256 S x=x\256 Q:x=0  S x(1)=x(1)+1
	.	F i=1:1 Q:x(i)<256  S x(i)=x(i)#256 S x(i+1)=$get(x(i+1))+1
	.	;
	.	S n=$order(x(""),-1)
	.	S len=$char(0)_$char(n)
	.	F i=n:-1:1 S len=len_$char(x(i))
	.	Q 
	;
	E  S len=$char(x)
	Q len_val
	;
LV2V(msg,arr,ptr,num)	;
	;
	...
	.	;
	.	S ln=$ascii($E(msg,ptr))
	.	S n=1
	.	;
	.	I ln=0 D  Q:exit 
	..		S n=$ascii($E(msg,ptr+1))
	..		S ptr=ptr+2
	..		;
	..		; Msg terminator
	..		I n=0 S exit=1 S ptr="" Q 
	..		S x=1
	..		F i=n-1:-1:0 S ln=($ascii(msg,ptr+i)*x)+ln S x=x*256
	..		Q 
	.	;
	.	S fld=fld+1
	.	S sub=sub+1
	.	S arr(sub)=$E(msg,ptr+n,ptr+ln-1)
	.	S ptr=ptr+ln
	.	Q 
	;
	Q ptr
...
NEXTVAL	; Returns the highest key value +1
	...
	;
vSIG()	;
	Q "60472^43703^Pete Chenard^8946" ; Signature - LTD^TIME^USER^SIZE
