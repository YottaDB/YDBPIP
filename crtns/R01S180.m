R01S180	; DBSDFKF - File Relationship Report
	;
	; **** Routine compiled from DATA-QWIK Report DBSDFKF ****
	;
	...
	S %TAB("VIN2")="|255||[DBTBL1F]FKEYS|[DBTBL1F]:QU ""[DBTBL1F]FID=<<FID>>"":NOVAL||D EXT^DBSQRY||T|Foreign Key Reference|||||"
	;
...
V0	; External report entry point
	...
	N VWHERE
	N %TIM N CONAM N RID N RN N VL N VLOF N VRF N VSTATS N vCOL N vHDG N vc1 N vc10 N vc2 N vc3 N vc4 N vc5 N vc6 N vc7 N vc8 N vc9 N vovc1 N vovc2 N vovc3 N vrundate N vsysdate
	;
	N cuvar S cuvar=$$vDb2()
	 S cuvar=$G(^CUVAR("BANNER"))
	;
	S CONAM="DATA-QWIK 7.2"
	S ER=0 S RID="DBSDFKF" S RN="File Relationship Report"
	S VL=""
	;
	...
	..		N CHRSET S CHRSET=$$^UCIOENCD("Report","DBSDFKF","V0","*")
	...
	D INIT^%ZM()
	;
	S vCOL="[DBTBL1F]FKEYS#3#40,[DBTBL1F]TBLREF#45#25,[DBTBL1F]UPD#75#3,[DBTBL1F]DEL#80#3,[DBTBL1F]RCTOMIN#88#3,[DBTBL1F]RCTOMAX#93#3,[DBTBL1F]RCFRMIN#98#3,[DBTBL1F]RCFRMAX#103#3"
	;
	...
	.	I VIN2'="ALL" S DQQRY(SEQ)="[DBTBL1F]FKEYS "_VIN2 S SEQ=SEQ+1
	.	S FROM=$$DQJOIN^SQLCONV("DBTBL1F") Q:ER 
	...
	.	S VBNRINFO("PROMPTS",2)="WC2|"_"Foreign Key Reference"_"|VIN2|"_$get(VIN2)
	.	;
	.	D
	..		N SEQ
	..		N VALUE N VAR N X
	...
	..		Q 
	.	;
	.	S VBNRINFO("DESC")="File Relationship Report"
	.	S VBNRINFO("PGM")="R01S180"
	.	S VBNRINFO("RID")="DBSDFKF"
	.	S VBNRINFO("TABLES")="DBTBL1F"
	.	;
	.	S VBNRINFO("ORDERBY",1)="[SYSDEV,DBTBL1F]%LIBS"
	.	S VBNRINFO("ORDERBY",2)="[SYSDEV,DBTBL1F]1"
	.	S VBNRINFO("ORDERBY",3)="[SYSDEV,DBTBL1F]FID"
	.	S VBNRINFO("ORDERBY",4)="[SYSDEV,DBTBL1F]110"
	.	S VBNRINFO("ORDERBY",5)="[SYSDEV,DBTBL1F]FKEYS"
	.	;
	...
	N rwrs,vos1,vos2,sqlcur,exe,sqlcur,vd,vi,vsql,vsub S rwrs=$$vOpen0(.exe,.vsql,"DBTBL1F.%LIBS,DBTBL1F.FID,DBTBL1F.FKEYS,DBTBL1F.TBLREF,DBTBL1F.UPD,DBTBL1F.DEL,DBTBL1F.RCTOMIN,DBTBL1F.RCTOMAX,DBTBL1F.RCFRMIN,DBTBL1F.RCFRMAX","DBTBL1F",VWHERE,"DBTBL1F.%LIBS,DBTBL1F.FID,DBTBL1F.FKEYS","","/DQMODE=1")
...
VGETDATA(V,VI)	;
	...
	S vc4=$piece(V,$char(9),4) ; DBTBL1F.TBLREF
	S vc5=$piece(V,$char(9),5) ; DBTBL1F.UPD
	S vc6=$piece(V,$char(9),6) ; DBTBL1F.DEL
	S vc7=$piece(V,$char(9),7) ; DBTBL1F.RCTOMIN
	S vc8=$piece(V,$char(9),8) ; DBTBL1F.RCTOMAX
	S vc9=$piece(V,$char(9),9) ; DBTBL1F.RCFRMIN
	S vc10=$piece(V,$char(9),10) ; DBTBL1F.RCFRMAX
	Q 
	;
...
VBREAK	;
	...
	I 0!(vovc1'=vc1) S vs(3)=0 S vh(3)=0 S VD(1)=0 S vb2=1 S vb3=1 S vb4=1 S vb5=1
	I vb3!(vovc2'=vc2) S vs(5)=0 S vh(5)=0 S VD(3)=0 S vb4=1 S vb5=1
	Q 
	;
...
VDTL5	; Detail
	...
	;
	S VL=VL_$J("",2-$L(VL))_$E(vc3,1,40)
	S VL=VL_$J("",44-$L(VL))_$E(vc4,1,25)
	S VL=VL_$J("",74-$L(VL))_$J(vc5,3)
	S VL=VL_$J("",79-$L(VL))_$J(vc6,3)
	S VL=VL_$J("",87-$L(VL))_$J(vc7,3)
	S VL=VL_$J("",92-$L(VL))_$J(vc8,3)
	S VL=VL_$J("",97-$L(VL))_$J(vc9,3)
	S VL=VL_$J("",102-$L(VL))_$J(vc10,3)
	D VOM
	Q 
	;
VHDG0	; Page Header
	...
	S VL="  "_"(A)"
	D VOM
	S VL="File Name                                   Foreign (B)                                (A)->(B)  (B)->(A)"
	D VOM
	S VL="Foreign Key Reference                        Filename                     Upd  Del     Min  Max  Min  Max"
	D VOM
	S VL="  "_"----------------------------------------  ------------                  ---  ---     ---  ---  ---  ---"
	D VOM
	;
...
VRSUM	; Report Summary
	...
	S VL="------------"
	D VOM
	S V=(VT(0)+VT(1)+VT(2)+VT(3)+VT(4)+VT(5)) ; @CNT(0,N,12)
	S VL=$J(V,12)
	D VOM
	Q 
	;
