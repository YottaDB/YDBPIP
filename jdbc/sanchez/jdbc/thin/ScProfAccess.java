...
public class ScProfAccess implements ScDBAccess
{
    ...
    public int executeUpdate(ScDBStatement dBStatement)
        throws SQLException, Exception
        {
            ...
            catch (Exception e) {aiRowCount = Integer.valueOf("0");}
    ...
    public int fetch(ScDBStatement dBStatement)
        throws SQLException,Exception
        {
            ...
            else
            {
	            if (prapare == 1) 
	            {
	                if ((sTempResult.length==3)||((sTempResult.length==4)&&(sTempResult[3].length()<1))) describe(statement);
	                else {
           ...
           //ddl
            ...
            //mrpc
                ...
                try{
			        ...
			        int i = tokenCheck(e);
                    if (i == 1) result = i_hHim.ProfileMRPC (oToken, mrpcId, version, params, spv, psStoreForward,size); // 09/09/03 MKT XBAD Changes	//**MKT-XBAD
	            }
