    ...
    private void sqlQuilifierUpdate(int queryOrUpdate)
    {
        if (!hSqlQaulifier.isEmpty()){
   		        ...
   		        for (int i=1; i<hSqlQaulifier.size()+1;i++)
		        {
			        ...
			        else if (!o.equalsIgnoreCase("ROWS")) bTemp.append(",D").append(o).append("=").append('\''+value+'\'');
                ...
                if (queryOrUpdate==1) ((ScMsqlQueryStatement)dBStatement).sqlQuailfier = ((ScMsqlQueryStatement)dBStatement).sqlQuailfier + fetchSize + bTemp.toString(); 
                else ((ScMsqlDDLStatement)dBStatement).sqlQuailfier = ((ScMsqlDDLStatement)dBStatement).sqlQuailfier + bTemp.toString(); 
            }
    }
