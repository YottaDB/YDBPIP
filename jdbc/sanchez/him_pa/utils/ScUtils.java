...
public int buffer_to_int (int pi_size, byte buffer[], int offset) {
		...
		switch (pi_size) {
			case 0: {return 0; }
			case 1: {return unsigned(buffer[offset]); }
			case 2: {ii =  unsigned((unsigned(buffer[offset+0]) * 0x100));
					 ii += unsigned(buffer[offset+1]);
					 return ii;}
			case 3: {ii =  unsigned((unsigned(buffer[offset+0]) * 0x10000));
					 ii += unsigned(buffer[offset+1] * 0x100);
					 ii += unsigned(buffer[offset+2]);
					 return ii;
					}
			case 4: {ii =  unsigned((unsigned(buffer[offset+0]) * 0x1000000));
...
public static void int_to_buffer (int pi_size, int num, byte buffer[], int offset) {
		...
		switch (pi_size) {
				case 1: {
						  buffer[offset+0] = (byte)num; break;
						}
				case 2: {
...
//--------------------------------------------------------------------------------------------------------------------------
	...
	if (source_number + 1 <= MAX_BYTE_VAL)	{
		...
		if (source_number + 2 <= MAX_WORD_VAL) {
		  the_size = 2;
		  temp_number = source_number + 2;
	   }
	   else
		  if (source_number + 3 <= MAX_3BYTE_VAL) {
		     the_size = 3;
   ...
   if (fileEncoding != null && mess!=null) 
   {
	...
	public static String detectEncoding(byte[] defaultBytes)
	{
		...
		if(defaultBytes.length >= 2)
		{
			...
			if(defaultBytes[0] == (byte)0xFE && defaultBytes[1] == (byte)0xFF)
				return UTF16BE;
			else
			if(defaultBytes[0] == (byte)0x3C && defaultBytes[1] == (byte)0x00)
				return UTF16LE;
			else
			if(defaultBytes[0] == (byte)0x00 && defaultBytes[1] == (byte)0x3C)
				return UTF16BE;
			else
			if(defaultBytes[0] == (byte)0x3F && defaultBytes[1] == (byte)0x00)
				return UTF16LE;
			else
			if(defaultBytes[0] == (byte)0x00 && defaultBytes[1] == (byte)0x3F)
				return UTF16BE;
		}
