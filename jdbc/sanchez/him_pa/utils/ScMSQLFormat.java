    ...
    public static String host(String X)
    {
        ...
        while(wrd!="end!"){
            ...
            if ((wrd.indexOf("<")>=0)||(wrd.indexOf(">")>=0)||(wrd.indexOf("=")>=0)){
                ...
                wrd=atomHost(sToken);
                if ((wrd.indexOf("<")>=0)||(wrd.indexOf(">")>=0)||(wrd.indexOf("=")>=0)) wrd=atomHost(sToken);
...
private static int fileCount(String sqlStatement)
{
    ...
    else if (aTemp.indexOf("SELECT ")>= 0)
    {
        ...
        if (aTemp.indexOf("WHERE")>0) a_sFileName = aTemp.substring(aTemp.indexOf("FROM")+4,aTemp.indexOf("WHERE"));
...
throws  Exception,SQLException
{
    ...
    catch (NoSuchElementException e) { ScDBError.check_error(80, "ScMSQLFormat");} 
    if (aTemp.compareTo("*")!=0)
        ...
        try{
            while(parser1.hasMoreElements()){
                ...
                if (a_sData.indexOf('.')>0) a_sData = a_sData.substring(a_sData.indexOf('.')+1,a_sData.length()).trim();
